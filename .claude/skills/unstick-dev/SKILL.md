---
name: unstick-dev
description: Diagnose and recover a stuck local dev environment. Detects hung next-server, dead backend, swarm registry desync, runaway CPU loops, orphan Claude CLI sessions, and stale git worktrees. Reports findings; kills runaways only with confirmation.
---
...
```bash
cat /Users/andrewbarrett/annix/.claude-swarm/registry.json
```
...
### 6. Orphan Claude CLI sessions

Use the patched detection (excludes desktop app helpers — never run the loose `ps | grep claude` filter, it will flag macOS Claude.app helpers as orphans):
...
function isClaudeCliCommand(c) {
  if (/Claude\.app|claudefordesktop|claude-simulator|Squirrel|ShipIt/i.test(c)) return false;
...
- **Do not** use the loose `ps | grep claude` filter — it will match the macOS Claude desktop app helpers and offer to kill them. Always use the `isClaudeCliCommand` filter above.
