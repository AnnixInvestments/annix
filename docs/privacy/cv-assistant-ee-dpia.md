...
| Candidate forgets they disclosed | Low. | Low. | Seeker self-service page at `/cv-assistant/seeker/ee-attributes` shows current disclosure + privacy notice + withdraw button. |
...
A dedicated spec test (`cv-assistant/services/ee-firewall.spec.ts`, 64
...
| Right | How exercised |
|---|---|
| Access (POPIA s23) | Seeker page `/cv-assistant/seeker/ee-attributes` shows current disclosure. Data export at `GET /cv-assistant/me/data-export` includes EE attributes per candidacy. |
