...
(`cv-assistant-ee-dpia.md`) and the customer policy pack
(`cv-assistant-ee-customer-policy-pack.md`).

## Activation flow

1. **Sign the DPIA.** Both Annix's Information Officer and the customer's
   Information Officer countersign `cv-assistant-ee-dpia.md`. Until that
...
4. **Seed the customer's sectoral targets.** Use the admin endpoint
   `POST /admin/cv-assistant/ee-sectoral-targets` (one row per sector ×
...
- **Running EE reports.** Visit
  `/cv-assistant/portal/compliance/ee-report`, pick a date range, click
...
- **First disclosure.** Triggered by HR sending a tokenised email after
  application. Candidate clicks the link (`/cv-assistant/ee-disclosure/[token]`),
...
- **Self-service correction or withdrawal.** From the seeker portal at
  `/cv-assistant/seeker/ee-attributes` the candidate can view, update, or
  ...
  language privacy notice at `/cv-assistant/seeker/ee-attributes/privacy-notice`.
