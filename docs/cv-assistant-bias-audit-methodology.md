...
> `scripts/cv-assistant/bias-audit.mjs` is the deliverable; the audit itself is
...
- Output report must be stored in `scripts/cv-assistant/bias-audit-reports/`
...
DATABASE_URL="postgresql://annix_cv_ai:..." \
  node scripts/cv-assistant/bias-audit.mjs \
  --output scripts/cv-assistant/bias-audit-reports/$(date +%Y-%m-%d).md
```
