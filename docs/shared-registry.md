...
| CV Assistant SA location hierarchy (frontend) | `lib/cv-assistant/sa-locations.ts` | Thin re-export of `@annix/product-data/sa-market` for the existing frontend import path. Backend re-exports from the same package via `annix-backend/src/cv-assistant/config/sa-market.config.ts`. (ref #268, #279) |
