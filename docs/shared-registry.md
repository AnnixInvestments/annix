...
| HDPE | `hdpe/` | grades, SDR ratings, dimensions, temperature derating, welding, welding standards, standards, fittings, pricing, `sans1123BackingFlange`, `sans1123StubAssemblyDescription` (PN → SANS 1123 backing-flange table — used for HDPE stub-end assemblies on the BOQ), `equalTeeDimensions`, `reducingTeeDimensions`, `hdpeTeeDimensions`, `hdpeReducerLength`, `moulededHdpeElbowDimensions`, `lateralDimensions`, `hdpeEndCapLength` (PE100 SDR 11 butt-fusion fitting body geometry sourced from manufacturer catalogues — used by the BOQ row builder as a fallback when entry.specs hasn't captured the dims explicitly. Lateral coverage is DN 63-160 only — fabricated to drawing above that. End cap coverage is DN 50-800.) |
...
| Nix capability registry | `nix/capabilities/nix-capability-registry.service.ts` + `INixCapability` interface in `nix-capability.interface.ts` | Umbrella registry for cross-app Nix capabilities (extraction / chat / walkthrough). Apps register `INixCapability` instances on bootstrap. Each capability declares `key`, `appCode`, `intents[]`, optional `extractionProfile` (delegating to `IExtractionProfileHandler`), `guideSlug`, and `walkthrough` definition. Co-exists with `NixExtractionProfileRegistry` — capabilities are the umbrella, profiles are one specific role a capability fulfils. Phase 5 cross-app intent routing reads `findByIntent()`. (ref #262 Phase 1) |
| Nix walkthrough definition | `nix/capabilities/walkthrough-definition.interface.ts` | `IWalkthroughDefinition`, `WalkthroughStep`, `WalkthroughStepKind`. Phase 4 engine consumes these. Steps may be inline static or sourced from a how-to guide via `guideSlug` + `NixGuideLoader`. (ref #262 Phase 4 prep) |
| Nix guide loader | `nix/capabilities/nix-guide-loader.service.ts` | Parses how-to-guide markdown for walkthrough mode. Reads from `<NIX_GUIDES_ROOT>/<appCode>/how-to/guides/<slug>.md` (defaults to `../annix-frontend/src/app`). Returns `ParsedGuide` (front-matter + body + headings). Minimal YAML subset parser inline; Phase 6 polish replaces with shared `@annix/how-to` package. (ref #262 Phase 1) |
| Nix walkthrough engine | `nix/capabilities/walkthrough-engine.service.ts` | Drives step-by-step guidance. State persists on `nix_chat_sessions.walkthrough_state` (jsonb, migration `1820100000072`). Methods: `start(sessionId, capabilityKey)`, `advance/back/skip(sessionId)`, `stop(sessionId, reason?)`, `state(sessionId)`, `currentStepView(sessionId)`, `stuckContext(sessionId)`. Step source resolves from inline `walkthrough.steps[]` first, then guide H2 partitions via `NixGuideLoader`. (ref #262 Phase 4) |
| Nix walkthrough trigger detection | `nix/capabilities/nix-capability-registry.service.ts` (`matchWalkthroughIntent`) | Detects "walk me through X" / "step by step X" / "guide me through X" / "show me how to X" / "hold my hand" trigger phrases in chat messages, returns the matching capability + remainder. Naive substring match against intents and labels. (ref #262 Phase 4) |
| Nix walkthrough endpoints | `nix/controllers/nix-walkthrough.controller.ts` | `POST /nix/walkthrough/:sessionId/{start,advance,back,skip,stop,state,current-step}`. Thin pass-through to `WalkthroughEngine`. AnyUserAuthGuard. (ref #262 Phase 4) |
| Nix mine inference | `nix/mine-inference.service.ts` | Fuzzy-matches Gemini-extracted title-block metadata (project / customer / docNumber prefix) against `SaMine`. Confidence threshold 0.7 for auto-attach; persists `documentNumber` + `documentRevision` even on weak matches for global cross-quote lookup. Wired into `NixService.processDocument` + `retryExtraction`. (ref #264 Phase 1) |
| Nix mine library | `nix/mine-library/` | `MineLibraryService` + `MineLibraryController` exposing `GET /nix/mine-library/mines`, `GET /nix/mine-library/mines/:id/extractions`, `GET /nix/mine-library/extractions/by-doc-number?q=`, `POST /nix/mine-library/mines` (with optional retag), `PATCH /nix/mine-library/extractions/:id/mine`, `DELETE /nix/mine-library/extractions/:id/mine`. Frontend hooks: `useMineLibraryMines`, `useMineLibraryExtractions`, `useDocNumberSearch`, `useCreateMineLibraryMine`, `useRetagExtractionMine`, `useClearExtractionMine`. (ref #264 Phase 2) |
...
| Query hooks | `lib/query/hooks/<subject>/use<Subject>.ts` | ALL page-level data fetching must use these. Subjects: `admin`, `annix-rep`, `au-rubber`, `boq`, `comply-sa`, `customer`, `cv-assistant`, `drawing`, `nix`, `rfq`, `supplier`, `reference`. CV Assistant interview scheduling lives in `cv-assistant/useCvInterviewBooking.ts` (slot CRUD, send-invite mutation, seeker bookings/invites/calendar-advisory). |
...
| CV Assistant seeker job filters | `lib/cv-assistant/components/SeekerJobFilters.tsx` | Province / city / category / source / min-salary filter row for the seeker Browse Jobs feed. Province → city dropdown is dependent (city options come from `lib/cv-assistant/sa-locations.SA_LOCATION_HIERARCHY`). Category and source dropdowns disable when the active match list yields fewer than 2 distinct values. (ref #268) |
| CV Assistant SA location hierarchy (frontend) | `lib/cv-assistant/sa-locations.ts` | Province → cities lookup used by `SeekerJobFilters`. **Duplicated from** `annix-backend/src/cv-assistant/config/sa-market.config.ts.SA_LOCATION_HIERARCHY` pending extraction to a `packages/product-data/sa-market/` shared workspace package — currently no other frontend consumer needed it, so the duplication is justified-temporary. Extraction is a separate refactor. (ref #268) |
