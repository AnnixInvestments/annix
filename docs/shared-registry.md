...
| Annix Orbit external job viewer | `lib/annix-orbit/components/ExternalJobModal.tsx` | Full-page in-app popup (createPortal → body, `z-[9999]`) that renders an external job listing in an iframe under a branded header, so View & Apply keeps the seeker inside the app. Header has an always-available "Open in new tab" fallback (job boards that send X-Frame-Options/CSP won't render in the iframe) and an X that returns the user to where they were; Escape also closes. Used by the seeker Browse Jobs page for both matched and browse-all cards. |
