...
| Marketing site content | `marketing/` (`@annix/product-data/marketing`) | Typed content model + seed defaults for the annix.co.za ecosystem website (#338). Types: `MarketingSiteContent`, `MarketingHero`, `MarketingEcosystem`, `MarketingProduct`, `MarketingIndustries`, `MarketingIndustry`, `MarketingTrustBar`, `MarketingCtaBand`, `MarketingFooter`, `MarketingProductPage`, `MarketingProductFeature`, `MarketingLabs`, `MarketingAbout`, `MarketingSiteStatus`, `MarketingCta`, `MarketingStat`. `defaultMarketingContent()` returns the seed tree (ecosystem cards reference real `appKey` + `portalCode`). Single source of truth for the structured marketing content rendered by the public site and edited by the admin CMS. |
