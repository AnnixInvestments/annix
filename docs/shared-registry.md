...
| Rubber | `rubber/` | rubber products, lining recommendations, dimensions, coding types, calloff status, order status; **rubber lining pricing seed** (#377): `RUBBER_PRICE_PRODUCTS` / `RUBBER_PLATE_PRICE_PRODUCTS` / `RUBBER_PIPE_PRICE_PRODUCTS` (+ `RubberPriceProductSeed`, `RubberPriceProductFamily`), `RUBBER_BONDING_AGENTS` (`RubberBondingAgentSeed`), `RUBBER_LABOUR_RATES` (`RubberLabourRateSeed`), `RUBBER_PRICING_DEFAULTS` (`RubberPricingDefaults`, `RubberNbFactor` — paraffin/blasting/markup/waste/MPS/consumable-markup factors, plate/pipe thicknesses, NB circumference factors), `RUBBER_BONDING_COVERAGE_REFS` + `lookupRubberBondingCoverage(name)` (`RubberBondingCoverageRef`, `RubberBondingCoverageResult` — manufacturer-sourced adhesive coverage matched by product name: Impilo HeroBond/HeroPrime from their price list & application guide, Rema SC2000/SC4000/BC3000/PR200 from published TDS; drives the bonding-agents "Fill coverage from datasheets" backfill and the price-list import coverage fallback). Consumed by the AU Rubber pricing engine (`rubber-lining/services/rubber-pricing.service.ts` + `entities/rubber-pricing-config.ts`). |
...
| Marketing site content | `marketing/` (`@annix/product-data/marketing`) | Typed content model + seed defaults for the annix.co.za ecosystem website (#338). Types: `MarketingSiteContent`, `MarketingHero` (+ `MarketingHighlight`), `MarketingEcosystem`, `MarketingProduct` (has `appKey`, `portalCode`, `imageUrl`, `comingSoon`), `MarketingIndustries`/`MarketingIndustry` (industry tiles with `imageUrl`), `MarketingPartners`/`MarketingPartner` (CMS-managed partner logos), `MarketingGlobalPresence`/`MarketingPresenceItem`, `MarketingCtaBand`, `MarketingFooter` (+ `MarketingFooterColumn`, `MarketingSocialLink`), `MarketingProductPage`/`MarketingProductFeature`/`MarketingProductRoi`, `MarketingLabs`, `MarketingAbout`, `MarketingSiteStatus`, `MarketingCta`. `defaultMarketingContent()` returns the seed tree (ecosystem cards reference real `appKey` + `portalCode`). Single source of truth for the structured marketing content rendered by the site and edited by the admin CMS. **Locales (#363):** `MARKETING_LOCALES` (`en`/`af`/`zu`/`fr`/`pt`/`es`), `DEFAULT_MARKETING_LOCALE`, `MarketingLocale`, `MARKETING_LOCALE_META`, `isMarketingLocale()`, `normaliseMarketingLocale()`, `marketingLocaleMeta()` — the canonical locale set shared by the backend CMS and the frontend i18n layer. |
| CMS page blocks | `cms/` (`@annix/product-data/cms`) | Structured block model for multi-page website CMS (#361 — bringing AU Industries up to the Annix marketing block format). Discriminated union `CmsBlock` over `type`: `RichTextBlock`, `HeroBlock`, `FaqBlock` (+ `FaqItem`), `FeatureGridBlock` (+ `FeatureItem`), `CtaBandBlock`, `GalleryBlock`, `RelatedSolutionsBlock` (+ `RelatedItem`), `TrustBadgesBlock` (+ `TrustBadge`), `TwoColumnBlock`, `CaseStudyStripBlock`; shared `CmsLink`, `CmsImage`. Helpers: `CMS_BLOCK_TYPES` (editor metadata list), `defaultBlock(type)` (blank block factory), `blockTypeLabel(type)`. Rendered SSR by `annix-frontend/src/app/lib/cms/render/BlockRenderer.tsx` (+ `jsonLd.ts` → `faqJsonLdFromBlocks`); edited by the shared `annix-frontend/src/app/lib/cms/components/BlockEditor.tsx`. Backend stores blocks as untyped JSON arrays on `website_page` (`draftBlocks`/`publishedBlocks`/`useBlocks`) — see the AU website CMS service below. **Reuse this block model for any future structured-page CMS; do not redefine per-app block types.** |
...
| Office → PDF conversion | `lib/libreoffice-conversion.service.ts` | `LibreOfficeConversionService.convertToPdf(buffer, ext)` — headless LibreOffice (`soffice`) converts legacy `.doc`/`.rtf`/`.odt`/image-only `.docx` to PDF so the normal text-layer extraction (then Gemini OCR) can run. Returns `null` when `soffice` is absent (e.g. local dev) so callers fall back to native parsing. Requires `libreoffice-writer` in the Docker runner; binary resolved from `LIBREOFFICE_PATH` or common paths. |
...
| Notifications dispatcher | `notifications/` | Multi-channel dispatch (email, in-app, SMS, WhatsApp). The `whatsapp` channel sends through the shared `whatsapp/` module (Cloud API) and records every send in the admin inbox; pass `data.appContext` in `NotificationContent` to tag which app sent it. |
| WhatsApp (global number, Cloud API) | `whatsapp/` | The platform's ONE WhatsApp number on Meta's Cloud API — no phone, no Twilio. `WhatsAppCloudApiService.sendText()` for sends; `WhatsAppConversationService` stores conversations/messages (collections `whatsapp_conversations`/`whatsapp_messages`, core cluster) and powers the global Admin → WhatsApp inbox (`/admin/portal/whatsapp`, `AdminWhatsAppController`). Inbound arrives via the public webhook `GET/POST /public/whatsapp/webhook`. Env (prod-only Fly secrets): `WHATSAPP_ACCESS_TOKEN`, `WHATSAPP_PHONE_NUMBER_ID`, `WHATSAPP_WEBHOOK_VERIFY_TOKEN`. Apps never call the Graph API directly — send via the notifications dispatcher (`whatsapp` channel) or this module. |
...
| Marketing site CMS (draft→publish) | `marketing/` | Singleton draft/published snapshot store for the annix.co.za ecosystem website (#338). `MarketingSiteContentService`: `draftContent(locale?)`, `publishedContent(locale?)`, `publishedLocales()`, `status()`, `saveDraft(content, locale?)`, `publish(by)`, `discardDraft()` — content is the typed `MarketingSiteContent` tree from `@annix/product-data/marketing` (lazily seeded from `defaultMarketingContent()`). **Multi-language (#363):** the entity carries `draftTranslations`/`publishedTranslations` (locale→tree maps); a non-`en` locale is overlaid on the English base so untranslated fields fall back to English. `publish` snapshots all locales together. **Auto-translation:** `MarketingTranslationService.translateDraftInto(locale)` walks the English draft, batch-translates every prose string via `AiChatService` (Gemini) — skipping identifier/URL/asset/colour keys and brand names — and saves it as that locale's draft (metric-timed `marketing/translate-content`; admin endpoint `POST /admin/marketing/translate?locale=`, button in the CMS editor driving an `ExtractionProgressModal`). Machine translation, reviewed before publish. **Public site is LIVE at the root host (annix.co.za):** `/` (home), `/products/[slug]`, `/industries/[slug]`, `/resources`, `/contact`, `/about`, `/labs` are server components that read the locale cookie via `loadMarketingContent()` (`lib/marketing/serverContent.ts`) and render the shared views inside `MarketingShell`. The global Annix nav is suppressed on these routes (`isMarketingSiteRoute` in `navigation-utils.ts`); the **launcher hub moved to `/portals`**. Legal pages: `/privacy`, `/terms`, `/cookies`. Admin authoring still at `/admin/portal/marketing` (editor + `/preview`, with a **Language** selector + Auto-translate). Read endpoints `GET /public/marketing/content?locale=` (cached 60s) + `GET /public/marketing/locales`. **Reuse this draft/publish mechanism for any other site that needs edit-then-publish (e.g. the unbuilt AU Industries go-live, #313).** |
...
| `SmartDropzone` (`@/app/lib/nix/components/upload`) | **Preferred for new Nix extraction flows (issue #266).** One dropzone for a mixed pack; Nix auto-classifies each file's role (drawing / specification / other) with a per-file chip + "reclassify" dropdown, single Process button. Pair with `useSmartDocuments()` (classification state; filename heuristic instantly, AI content glance for ambiguous PDFs, user override wins) and `extractionPassesOf()` (orders extraction drawings → specs → other so the spec cross-linking block keeps working). Backend: `RoleClassifierService` (`annix-backend/src/nix/services/role-classifier.service.ts`) behind `POST /nix/classify-role` (filename heuristics) and `POST /nix/classify-role/content` (AI glance). |

#### Website CMS editor (`app/lib/cms/components/`)

The shared admin editing UI for the multi-page website CMS (the block model lives in `@annix/product-data/cms`; see the reference-data table above). Consumed by the admin Marketing tab's AU pages (`app/admin/portal/marketing/au/**`) and any future app that edits CMS pages.

| Component | Use for |
|---|---|
| `BlockEditor` (`lib/cms/components/BlockEditor.tsx`) | The block builder: add/reorder/remove blocks and edit per-block-type fields (richText, hero, faq, featureGrid, ctaBand, relatedSolutions, trustBadges, gallery, twoColumn, caseStudyStrip); Save Draft / Publish Blocks; **Edit/Preview toggle** that renders the draft through `BlockRenderer`. Image fields use `ImageField`. Calls the admin-token `auCmsAdminApi`. Props `{ pageId, initialBlocks }`. |
| `ImageField` (`lib/cms/components/ImageField.tsx`) | Image input with paste-a-URL **or** upload-a-file (uploads via `auCmsAdminApi.uploadWebsiteImage`) plus a thumbnail preview. Props `{ value, onChange, onError? }`. Reuse for any CMS block image slot. |
| `HeroImagePicker` (`lib/cms/components/HeroImagePicker.tsx`) | Modal (portal-to-body) gallery picker for choosing a hero banner from the bundled AU Industries image set. Props `{ currentImage, onSelect }`. |

The CMS data clients/hooks are the admin-token `auCmsAdminApi` (`app/lib/api/auCmsAdminApi.ts`) + the `useAuCms*` hooks (`lib/query/hooks/au-cms/useAuCms.ts`, barrelled from `@/app/lib/query/hooks`) with keys in `lib/query/keys/auCmsKeys.ts`.
...
| Floating-panel geometry | `lib/panelGeometry.ts` | `PanelGeometry` type + `clampPanelGeometryToViewport(geometry, minWidth, minHeight)` — clamps a persisted draggable-panel position/size to the current viewport (returns `null` for non-finite/corrupt values so callers fall back to their default). MANDATORY for any floating panel that restores its position from localStorage (FeedbackWidget, NixChatPanel) — an unclamped restore renders the panel off-screen after a viewport/zoom change and the widget "disappears". |
| Screenshot exclusion | `lib/screenshotExclusion.ts` | `SCREENSHOT_EXCLUDE_ATTRIBUTE` / `SCREENSHOT_EXCLUDE_SELECTOR` / `screenshotExcludeProps` — spread `screenshotExcludeProps` onto any purely-decorative heavy layer (brand page-background, watermark) so the FeedbackWidget's html-to-image capture skips it. Keeps feedback screenshots fast (the brand layers made captures 4-5× slower and time out) and small (3.1MB → ~0.5MB). Applied by `GlobalBrandBackground` + `BrandingProvider`; add it to any new full-page decorative image layer. |
...
| Marketing i18n | `lib/marketing/i18n/` | Multi-language layer for the marketing site (#363). `<MarketingI18nProvider>` (cookie `annix_locale` + `?lang=` query, controlled or uncontrolled), `useMarketingTranslations(namespace)` → `t(key)` with English fallback, `useMarketingLocale()` (`locale`/`setLocale`), `<MarketingLanguageSwitcher>` (nav + mobile), `<MarketingHreflangLinks>`, message catalogs `messages/{en,af,zu,fr,pt,es}.ts` (UI **chrome** only — nav/cookies/contact/footer/legal; CMS body copy is translated per-locale in the CMS, not here). Locale set comes from `@annix/product-data/marketing`. Mounted once by `MarketingShell`; do not build a parallel i18n system for marketing. |
...
| Annix Orbit admin email outreach | `components/orbit/OrbitEmailComposer.tsx`, `lib/query/hooks/admin/useAdminOrbitOutreach.ts`, backend `annix-orbit/services/orbit-outreach.service.ts` + `controllers/admin-orbit-outreach.controller.ts` + `entities/schemas orbit-outreach-asset.*` | **Orbit-admin-specific.** Reusable admin email composer mounted by BOTH the Early Access (`app/admin/portal/orbit/early-access`) and Orbit Users (`app/admin/portal/orbit/users`) pages — recipient tick-list + "send to all", subject/body, prod/test link environment, device-aware install-guide attachment (matching iPhone/Android guide, or both as body links when device unknown) + Feedback-Widget guide + extra attachments. Sends per-recipient (real progress) via `POST /admin/annix-orbit/outreach/send` from `noreply@` with `admin@annix.co.za` (config `ORBIT_OUTREACH_REPLY_TO`) reply-to + branded signature. Central guides/attachments are stored once (S3 + `cv_assistant_outreach_assets` on the Orbit cluster) and managed inside the composer — do NOT add a parallel composer or a per-page guide store. Also supports **send now or schedule for a future date/time** (`cv_assistant_outreach_schedules` + the `annix-orbit:outreach-dispatch` 15-min cron, Orbit-cron-gated). Mount it with `recipients`/`trackEarlyAccess`/`contextLabel`. |
...
| Annix Orbit external-job embeddings (Orbit cluster collection) | `cv_assistant_external_job_embeddings` (schema `annix-orbit/schemas/external-job-embedding.schema.ts`) | Sibling collection on the **Orbit** Mongo cluster holding the per-job packed Float32 embedding `Buffer`, keyed by the parent job's numeric `_id` (1:1, no shared `counters` sequence — writes upsert by explicit `_id`). Split out of `cv_assistant_external_jobs` so the hot job doc stays small (~7 KB → ~4 KB). It is a **retention fan-out sibling** alongside `cv_assistant_candidate_job_matches` / `cv_assistant_external_job_alternates`: EVERY job-delete path in `MongoExternalJobRepository` (`deleteById`, `deleteByIds`, `expireStaleJobs`, `trimToFreshest`, `enforceRetentionCap`) must also delete the matching embedding(s) or the matcher would score orphans. Read via `jobEmbedding(id)` / `jobEmbeddings(ids)`; the embedding scan (`jobEmbeddingBatches`) cursors this collection directly. |
