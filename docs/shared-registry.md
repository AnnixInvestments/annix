...
| Nix extraction profile registry | `nix/profiles/nix-extraction-profile-registry.service.ts` | Apps register their own document-extraction profile with a `profileKey` (e.g. `rfq-piping`, `asca-quote-documents`). Each handler implements `IExtractionProfileHandler` (`profileKey`, `label`, `sourceModule`, `postExtract`, optional `systemPrompt`) and registers itself via `OnModuleInit`. Uses the same self-registering pattern as `SageAdapterRegistry`. The Nix service routes `processDocument` post-extraction work through the registered handler when `extractionProfile` is supplied. (ref #251) |
