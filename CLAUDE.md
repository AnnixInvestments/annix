...
6. **ALWAYS watch every push through to a successful deploy — never fire-and-forget.** A push is not "done" when `git push` returns; it's done when the GitHub Actions deploy run (Build → Tests → Plan → Deploy staging [runs `release_command`] → test → prod) goes green. After any push, find the run (`gh run list --branch main`) and watch it to completion (background watcher polling `gh run view <id> --json status,conclusion,jobs`). **If the deploy fails, automatically diagnose, fix, and re-push until it goes green — no need to ask first.** This auto-fix-and-repush loop is part of fulfilling the *original* push instruction (the same way fixing a pre-push hook failure is, per rule 3); it does NOT need a fresh push instruction for each retry. The authorization is consumed only once the deploy is green (or the user redirects). Report the final deploy verdict (per-job table) without being asked. This rule does not loosen rules 1–2: a *first* push still needs its own explicit instruction.
