...
const normalizePressureClass = (designation: string | null | undefined): string => {
  ...
  if (sabsMatch) {
    const kpa = parseInt(sabsMatch[1], 10);
...
const getFlangeWeight = (
  ...
  if (defaultWeight)
    return { weight: defaultWeight, normalized: pressureClass, found: false, fallback: "PN16" };
...
console.log(
  `  Global (${globalPressureClass}): ${globalResult.weight}kg × 2 = ${globalResult.weight * 2}kg total`,
);
console.log(
  `  Override (${overridePressureClass}): ${overrideResult.weight}kg × 2 = ${overrideResult.weight * 2}kg total`,
);
...
console.log("=".repeat(80));
console.log(
  `Normalization Tests: ${TEST_DESIGNATIONS.length - normErrors}/${TEST_DESIGNATIONS.length} passed`,
);
