...
import { guideVisibleToRole, type HowToHeading, type HowToLink } from "../types";
...
export interface HowToViewerClientProps {
  guide: ViewerGuide;
...
export default function HowToViewerClient(props: HowToViewerClientProps) {
  ...
  const visible = useMemo(
    () => guideVisibleToRole(props.guide, props.role, props.allRoles),
    [props.guide, props.role, props.allRoles],
  );

  useEffect(() => {
    if (!visible && props.role !== null) {
      router.replace(props.basePath);
    }
  }, [visible, props.role, props.basePath, router]);
  ...
  }, [props.guide.slug, props.recentKey]);

  useEffect(() => {
    try {
      const stored = localStorage.getItem(`${props.helpfulKeyPrefix}${props.guide.slug}`);
      if (stored === "yes" || stored === "no") setHelpful(stored);
    } catch (e) {
      console.warn("Failed to read helpful state", e);
    }
  }, [props.guide.slug, props.helpfulKeyPrefix]);

  const recordHelpful = (value: "yes" | "no") => {
    setHelpful(value);
    try {
      localStorage.setItem(`${props.helpfulKeyPrefix}${props.guide.slug}`, value);
    } catch (e) {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
      <nav className="text-xs text-gray-500 mb-4 flex items-center gap-1 flex-wrap">
        <Link href={props.basePath} className="hover:text-teal-600">
          How To
        </Link>
        <span>/</span>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[1fr_220px] gap-8">
        <article className="bg-white rounded-lg shadow-sm border border-gray-200 p-6 sm:p-8 print:shadow-none print:border-0">
          ...
          <div className="mt-6 flex items-center justify-between gap-4 print:hidden">
            {props.prev ? (
              <Link
                href={`${props.basePath}/${props.prev.slug}`}
            ...
            {props.next ? (
              <Link
                href={`${props.basePath}/${props.next.slug}`}
