...
class AdminCustomerApiClient {
  ...
  async getLoginHistory(
    customerId: number,
    limit: number = 50,
  ): Promise<CustomerLoginHistoryItem[]> {
    return this.request<CustomerLoginHistoryItem[]>(
      `/admin/customers/${customerId}/login-history?limit=${limit}`,
    );
  }
}
...
export const adminCustomerApi = {
  ...
  getCustomerDetail: (customerId: number) => adminCustomerApiClient.getCustomerDetail(customerId),
