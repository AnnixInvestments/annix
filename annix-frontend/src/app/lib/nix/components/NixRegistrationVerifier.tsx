...
export default function NixRegistrationVerifier({
  ...
  const [annotatingField, setAnnotatingField] = useState<{
    fieldName: string;
    label: string;
  } | null>(null);
  ...
  const handleApplySelected = () => {
    ...
    const corrections = verificationResult.autoCorrections.filter((c) =>
      selectedCorrections.has(c.field),
    );
      ...
      <div className="flex items-center gap-4 mb-6">
        ...
        <div className="flex-1 text-left">
          <h2 className="text-lg font-bold text-gray-900">
            Nix is Verifying Your {DOCUMENT_TYPE_LABELS[documentType]}
            {dots}
          </h2>
      ...
      <div className="px-6 py-6 max-h-[80vh] overflow-y-auto">
          ...
          <div className="mb-4">
            ...
            <div className="space-y-3">
              {mismatches.map((field) => {
                const hasAutoCorrect = verificationResult.autoCorrections.some(
        ...
        <div className="space-y-2">
          {selectedCorrections.size > 0 && (
            ...
            >
              Apply {selectedCorrections.size} Correction{selectedCorrections.size !== 1 ? "s" : ""}{" "}
              & Continue
            </button>
          )}
          <button
    ...
    <div className="px-6 py-6">
      <div className="flex items-center gap-4 mb-6">
        ...
        <div className="flex-1 text-left">
          ...
          <p className="text-sm text-gray-600 mt-1">The document could not be processed</p>
        </div>
      </div>
      ...
      <div className="bg-red-50 border border-red-200 rounded-lg p-4 mb-4">
        <p className="text-sm text-red-700">
          {verificationResult?.warnings.join(". ") ||
            "Document extraction failed. Please ensure the document is clear and readable."}
        </p>
      </div>
