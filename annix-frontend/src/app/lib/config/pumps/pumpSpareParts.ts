...
export const PUMP_OEM_MANUFACTURERS: OemManufacturer[] = [
  { value: 'ksb', label: 'KSB', country: 'Germany', commonModels: ['Etanorm', 'Etabloc', 'Omega', 'Movitec'] },
  { value: 'grundfos', label: 'Grundfos', country: 'Denmark', commonModels: ['CR', 'CRN', 'NB', 'NK', 'SE', 'SL'] },
  { value: 'sulzer', label: 'Sulzer', country: 'Switzerland', commonModels: ['Ahlstar', 'CPT', 'HPT', 'MBN'] },
  { value: 'flowserve', label: 'Flowserve', country: 'USA', commonModels: ['Durco', 'Worthington', 'Byron Jackson'] },
  { value: 'weir', label: 'Weir Minerals', country: 'UK', commonModels: ['Warman', 'Envirotech', 'Geho'] },
  { value: 'itt_goulds', label: 'ITT Goulds', country: 'USA', commonModels: ['3196', '3296', 'IC', 'LF'] },
  { value: 'ebara', label: 'Ebara', country: 'Japan', commonModels: ['FSA', 'FHA', 'DL', 'DS'] },
  { value: 'pentair', label: 'Pentair / Aurora', country: 'USA', commonModels: ['Aurora', 'Fairbanks Nijhuis'] },
  { value: 'ruhrpumpen', label: 'Ruhrpumpen', country: 'Germany', commonModels: ['SCE', 'SCP', 'ZW'] },
  { value: 'andritz', label: 'Andritz', country: 'Austria', commonModels: ['A', 'ACP', 'S'] },
  { value: 'alfa_laval', label: 'Alfa Laval', country: 'Sweden', commonModels: ['LKH', 'SRU', 'SX'] },
  { value: 'spx_flow', label: 'SPX Flow', country: 'USA', commonModels: ['W+', 'Universal', 'Johnson'] },
  { value: 'sterling', label: 'Sterling SIHI', country: 'Germany', commonModels: ['ZLND', 'CEH', 'LPH'] },
  { value: 'local_za', label: 'Local (South Africa)', country: 'South Africa', commonModels: ['Various'] },
];
...
export const PART_NUMBER_FORMATS: PartNumberFormat[] = [
  { oem: 'ksb', format: 'XXXXX-XXX-XX', example: '48293-001-05', description: 'KSB uses 5-digit base, 3-digit variant, 2-digit size' },
  { oem: 'grundfos', format: 'XX-XXXXXXXX', example: '96-94567812', description: 'Grundfos uses 2-digit prefix and 8-digit serial' },
  { oem: 'flowserve', format: 'XXX-XXXXX', example: 'DUR-12345', description: 'Flowserve uses brand prefix and 5-digit part number' },
  { oem: 'weir', format: 'AXXXX-XXX-XXX', example: 'A05518-001-001', description: 'Weir uses letter prefix, 5-digit part, variants' },
];
...
export const BEARING_TYPES: BearingType[] = [
  {
    ...
    advantages: ['High speed capability', 'Low friction', 'Low maintenance', 'Handles combined loads'],
  ...
  },
  {
    ...
    loadCapacity: 'radial',
    speedCapability: 'high',
    advantages: ['Very high radial load capacity', 'Good for heavy loads', 'Allows axial displacement'],
  ...
  },
  {
    ...
    typicalApplications: ['Slurry pumps', 'Heavy industrial pumps', 'Pumps with flexible couplings'],
  },
  {
  ...
  },
  {
    ...
    loadCapacity: 'radial',
    speedCapability: 'medium',
    advantages: ['Excellent chemical resistance', 'Good for abrasives', 'High temperature capability'],
...
export const STANDARD_IMPELLER_TRIMS: ImpellerTrimOption[] = [
  { value: 'max', label: 'Maximum Diameter', percentOfMax: 100, description: 'Full size impeller - maximum head', efficiencyImpact: 'Optimal efficiency at design point' },
  { value: 'trim_95', label: '95% Trim', percentOfMax: 95, description: 'Light trim for minor adjustment', efficiencyImpact: 'Minimal efficiency loss (~1%)' },
  { value: 'trim_90', label: '90% Trim', percentOfMax: 90, description: 'Moderate trim', efficiencyImpact: 'Slight efficiency loss (~2-3%)' },
  { value: 'trim_85', label: '85% Trim', percentOfMax: 85, description: 'Standard trim range', efficiencyImpact: 'Moderate efficiency loss (~3-5%)' },
  { value: 'trim_80', label: '80% Trim', percentOfMax: 80, description: 'Significant trim', efficiencyImpact: 'Notable efficiency loss (~5-7%)' },
  { value: 'trim_75', label: '75% Trim', percentOfMax: 75, description: 'Heavy trim - near minimum', efficiencyImpact: 'Significant efficiency loss (~7-10%)' },
  { value: 'min', label: 'Minimum Diameter', percentOfMax: 70, description: 'Minimum allowable trim', efficiencyImpact: 'Maximum efficiency penalty - consider smaller pump' },
];
...
export const calculateTrimmedPerformance = (
  ...
  return {
    ...
    headM: Math.round(originalHeadM * Math.pow(ratio, 2) * 100) / 100,
...
export const recommendImpellerTrim = (
  ...
  maxHeadM: number,
  minHeadM: number
): { recommendedTrim: ImpellerTrimOption; estimatedDiameterPercent: number; warning: string | null } => {
...
export const API_610_WEAR_RING_CLEARANCES: WearRingClearance[] = [
  ...
  },
  {
    ...
    newClearanceMm: { min: 0.30, max: 0.38 },
  ...
  },
  {
    ...
    newClearanceMm: { min: 0.40, max: 0.50 },
    maxAllowedClearanceMm: 1.00,
    notes: 'Larger pumps',
  },
  {
    diameterRangeMm: { min: 200, max: 250 },
    newClearanceMm: { min: 0.45, max: 0.55 },
    maxAllowedClearanceMm: 1.10,
    notes: 'Larger pumps',
  },
  {
    diameterRangeMm: { min: 250, max: 300 },
    newClearanceMm: { min: 0.50, max: 0.60 },
    maxAllowedClearanceMm: 1.20,
    notes: 'Large pumps',
  },
  {
    diameterRangeMm: { min: 300, max: 400 },
    newClearanceMm: { min: 0.55, max: 0.70 },
    maxAllowedClearanceMm: 1.40,
    notes: 'Large pumps',
  },
  {
    diameterRangeMm: { min: 400, max: 500 },
    newClearanceMm: { min: 0.65, max: 0.80 },
    maxAllowedClearanceMm: 1.60,
...
export const assessWearRingCondition = (
  ...
  if (!spec) {
    return { status: 'monitor', message: 'Diameter outside standard range - consult OEM', efficiencyLoss: 0 };
  }
