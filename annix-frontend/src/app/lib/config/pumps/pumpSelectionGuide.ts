...
export const APPLICATION_PROFILES: ApplicationProfile[] = [
  ...
  },
  {
    ...
    recommendedPumpTypes: ['mag_drive', 'lined_centrifugal', 'diaphragm', 'peristaltic', 'air_operated_double_diaphragm'],
    considerations: [
  ...
  },
  {
    ...
    recommendedPumpTypes: ['sanitary_centrifugal', 'lobe', 'progressive_cavity', 'peristaltic', 'sine'],
    considerations: [
...
export const selectPumpType = (criteria: SelectionCriteria): SelectionResult => {
  ...
  const recommendations = PUMP_TYPE_PROFILES.map((pumpType) => {
    ...
    if (criteria.application) {
      ...
      if (appProfile?.recommendedPumpTypes.some((t) => pumpType.value.includes(t) || t.includes(pumpType.value))) {
        score += 20;
