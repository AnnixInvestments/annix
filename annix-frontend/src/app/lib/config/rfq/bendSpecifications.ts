 ...
 */
export const isNominalBoreValidForSpec = (steelSpecName: string, nominalBoreMm: number): boolean => {
  const rules = steelStandardBendRules(steelSpecName)
  if (!rules) return true
...
export const sabs62PipeData = (
  ...
  return (
    SABS_62_PIPE_DATA.find(
      (p) => p.nominalBoreMm === nominalBoreMm && p.pipeClass === pipeClass
    ) || null
  )
}
 ...
 */
export const segmentedBendDeratingFactor = (numberOfSegments: number, totalAngleDegrees: number): number => {
  ...
  const matchingDerating = SEGMENTED_BEND_DERATING.find(
    (d) =>
      d.numberOfMiters === numberOfMiters &&
      Math.abs(d.miterAngleDegrees - miterAngle) < 5
  )
...
export const recommendedBendType = (
  ...
  return {
    recommended: 'pulled',
    reason: 'Pulled bends generally preferred for better flow characteristics and pressure retention'
  }
}
