 ...
 */
export function calculateTotalLengthFromQuantity(quantity: number, pipeLengthMm: number): number {
  const pipeLengthMeters = pipeLengthMm / 1000;
 ...
 */
export async function getAvailableFlangeClasses(
  flangeStandardId: number,
): Promise<Array<{ id: number; designation: string }>> {
  try {
    // Import API client dynamically to avoid circular dependencies
    const { masterDataApi } = await import("../api/client");
 ...
 */
export async function getWallThicknessForSchedule(
  ...
  try {
    ...
    const dimension = pipeDimensions.find(
      (d) =>
 ...
 */
export async function getPipeEndConfigurationDetails(
  configCode: string,
): Promise<{ weldCount: number; description: string } | null> {
  try {
