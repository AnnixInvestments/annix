...
async function fetchDrawingDetail(id: number): Promise<DrawingDetail> {
  ...
  if (!response.ok) {
    const body = await response.text().catch(() => '')
    throw new Error(
      `Failed to fetch drawing (${response.status}: ${body || response.statusText})`,
    )
  }
...
export function useAddDrawingComment(drawingId: number) {
  ...
  return useMutation({
    mutationFn: async (commentText: string) => {
      const response = await fetch(
        `${browserBaseUrl()}/drawings/${drawingId}/comments`,
        {
          method: 'POST',
          headers: {
...
export function useUploadDrawingVersion(drawingId: number) {
  ...
  return useMutation({
    mutationFn: async ({
      ...
      const response = await fetch(
        `${browserBaseUrl()}/drawings/${drawingId}/version`,
        {
          method: 'POST',
          headers: getAuthHeaders(),
          body: formData,
...
export function useAnalyzeDrawing(drawingId: number) {
  return useMutation({
    mutationFn: async (): Promise<AnalysisResult> => {
      const response = await fetch(
        `${browserBaseUrl()}/drawings/${drawingId}/analyze`,
        { headers: getAuthHeaders() },
      )
