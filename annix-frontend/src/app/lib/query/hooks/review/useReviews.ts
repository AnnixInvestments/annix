...
async function fetchReviews(
  ...
  if (!response.ok) {
    ...
    throw new Error(`Failed to fetch reviews (${response.status}: ${body || response.statusText})`);
  }

  return response.json();
}

export function useReviews(tab: "pending" | "history", params?: ReviewQueryParams) {
  const queryKey = tab === "pending" ? reviewKeys.pending(params) : reviewKeys.history(params);
