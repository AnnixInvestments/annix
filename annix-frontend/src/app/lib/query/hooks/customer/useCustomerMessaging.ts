...
export function useCreateCustomerConversation() {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: (dto: CreateConversationDto) => customerMessagingApi.createConversation(dto),
    onSuccess: () => {
      queryClient.invalidateQueries({
        queryKey: customerKeys.messaging.conversations(),
      });
...
export function useMarkCustomerBroadcastRead() {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: (broadcastId: number) => customerMessagingApi.markBroadcastRead(broadcastId),
    onSuccess: () => {
      queryClient.invalidateQueries({
        queryKey: customerKeys.messaging.broadcasts(),
      });
