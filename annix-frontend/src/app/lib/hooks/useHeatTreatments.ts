...
export interface UseHeatTreatmentsReturn {
  ...
  checkRequirement: (
    request: HeatTreatmentRequirementRequest,
  ) => Promise<HeatTreatmentRequirementResponse | null>;
...
export function useHeatTreatments(): UseHeatTreatmentsReturn {
  ...
  const treatmentByCode = useCallback(
    (code: string): HeatTreatment | null => {
      return treatments.find((t) => t.code === code) || null;
    },
    [treatments],
  );

  const treatmentsByType = useCallback(
    (type: string): HeatTreatment[] => {
