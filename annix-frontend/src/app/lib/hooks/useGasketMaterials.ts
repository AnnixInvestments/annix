...
export interface UseGasketMaterialsReturn {
  ...
  checkCompatibility: (
    request: GasketCompatibilityRequest,
  ) => Promise<GasketCompatibilityResponse | null>;
...
export function useGasketMaterials(): UseGasketMaterialsReturn {
  ...
  const materialByCode = useCallback(
    (code: string): GasketMaterial | null => {
      return materials.find((m) => m.code === code) || null;
    },
    [materials],
  );

  const materialsByType = useCallback(
    (type: string): GasketMaterial[] => {
  ...
  const checkCompatibility = useCallback(
    async (request: GasketCompatibilityRequest): Promise<GasketCompatibilityResponse | null> => {
