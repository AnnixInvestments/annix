...
describe("Pump Selection Guide", () => {
  ...
  describe("selectPumpType", () => {
    ...
    it("should penalize pumps when flow exceeds maximum", () => {
      ...
      const endSuctionNormal = normalResult.recommendedTypes.find(
        (r) => r.type.value === "end_suction",
      );
      const endSuctionHigh = highFlowResult.recommendedTypes.find(
        (r) => r.type.value === "end_suction",
      );
    ...
    it("should add considerations for variable flow operation", () => {
      ...
      const hasVfdNote = result.considerations.some(
        (note) => note.toLowerCase().includes("vfd") || note.toLowerCase().includes("variable"),
      );
    ...
    it("should add considerations for chemical fluids", () => {
      ...
      const hasChemicalNote = result.considerations.some(
        (note) =>
    ...
    it("should generate warnings for centrifugal pumps when solids present", () => {
      ...
      const centrifugalPumps = result.recommendedTypes.filter(
        (r) =>
          r.type.category === "centrifugal" &&
          r.type.value !== "slurry" &&
          r.type.value !== "submersible",
      );
      const hasAnyWarning = centrifugalPumps.some((pump) =>
        pump.warnings.some(
          (w) => w.toLowerCase().includes("solids") || w.toLowerCase().includes("wear"),
        ),
      );
