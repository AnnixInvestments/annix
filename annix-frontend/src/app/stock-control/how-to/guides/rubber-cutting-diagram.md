...
On the **Line Items** tab, the **Re-extract** button rebuilds the line items from the source job card and now also runs AI extraction over the uploaded tank/chute drawings to pull each item's coating and lining **m²** — so items previously flagged as **missing m²** are filled in from the drawing automatically. Because this analyses multiple PDFs it can take a while, so a branded progress popup (Nix analysing) shows while it runs and closes when the re-extract finishes. When it completes, the line item m² and the rubber cutting plan update together. If some items are still flagged **missing m²** afterwards, the drawing may not show that area — edit the item and enter the m² manually (or add e.g. "@ 12.5m²" to the Sage item description).
