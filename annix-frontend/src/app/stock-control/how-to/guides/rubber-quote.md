...
summary: Quote rubber lining work fast — pick a rubber, bonding system, thickness and area (plate) or length and NB size (pipe), and read off the live sale and MPS price plus the line total.
readingMinutes: 2
...
Switching the toggle clears the current selection so you always quote against the right family.

## Quoting

1. **Supplier** — optionally narrow the rubber dropdown to one supplier, or leave it on **All suppliers**.
2. **Rubber** — pick a rubber from the dropdown. Only rubbers priced for the selected family appear.
3. **Bonding system** — defaults to the selected rubber's own bonding type. Change it to price the same compound under a different bonding system (for example precured), or leave it on **Default** to use the rubber's configured bonding.
4. **Thickness (mm)** — choose the lining thickness from the rubber's available thicknesses. It defaults to the thickest option.
5. **NB size** (pipe only) — choose the nominal bore (50NB … 1000NB). This drives the per-running-metre circumference factor, so a quote isn't produced until an NB size is selected.
6. **Area (m²)** (plate) or **Length (m)** (pipe) — enter the quantity to apply.
