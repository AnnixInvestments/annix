...
export function DeliveryNoteConfirmationModal(props: DeliveryNoteConfirmationModalProps) {
  ...
  const resolvedTitle = title ?? "Confirm Delivery Note";
  const resolvedSubtitle = subtitle ?? "Review and edit the extracted data before saving";
  const resolvedConfirmLabel = confirmLabel ?? "Confirm & Add to Stock";
  const allowSaveForReview = showSaveForReview ?? true;
