...
export default function ReportsPage() {
  ...
  useEffect(() => {
    if (activeTab === "movement-history" && !isLoading && !error && movements.length === 0) {
      setShowNoMovementsPopup(true);
    } else {
      setShowNoMovementsPopup(false);
    }
  }, [activeTab, isLoading, error, movements.length]);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="border-b border-gray-200">
        <nav className="-mb-px grid grid-cols-2 gap-0 sm:flex sm:space-x-8">
          {REPORT_TABS.map((tab) => (
            <button
              key={tab}
