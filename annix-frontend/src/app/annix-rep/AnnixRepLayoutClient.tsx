...
function AnnixRepNavigation() {
  ...
  const settingsNavItem = baseNavItems[baseNavItems.length - 1];
  const navItems = isManagerOrAdmin
    ? [...baseNavItems.slice(0, -1), managerNavItem, settingsNavItem]
    : baseNavItems;
  ...
  return (
    <PortalToolbar
      ...
      onLogout={async () => {
        await logout();
        window.location.href = "/annix-pulse/welcome";
      }}
      featureFlags={flags}
...
function SetupGate({ children }: { children: React.ReactNode }) {
  ...
  const isWelcomePage = pathname === "/annix-pulse/welcome";
  const isLoginPage = pathname === "/annix-pulse/login";
  const isSetupPage = pathname === "/annix-pulse/setup";
  const isSettingsPage = pathname?.startsWith("/annix-pulse/settings");
  const isOAuthCallback = pathname === "/annix-pulse/oauth/callback";
  ...
  useEffect(() => {
    ...
    if (!isAuthenticated && !isAuthPage) {
      router.replace("/annix-pulse/welcome");
      return;
    }
...
function LayoutContent({ children }: { children: React.ReactNode }) {
  ...
  const pathname = usePathname();
  const isAuthPage =
    pathname === "/annix-pulse/setup" ||
    pathname === "/annix-pulse/welcome" ||
    pathname === "/annix-pulse/login";
