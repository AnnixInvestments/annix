...
import Breadcrumbs from "@/app/annix-sentinel/components/Breadcrumbs";
import HelpLinks from "@/app/annix-sentinel/components/HelpLinks";
...
const TOP_LEVEL_LINKS: NavItem[] = [
  { href: "/annix-sentinel/dashboard", label: "Dashboard", icon: LayoutDashboard },
  { href: "/annix-sentinel/regulatory", label: "Regulatory Updates", icon: Newspaper },
];
...
const DROPDOWN_GROUPS: NavGroup[] = [
  {
    label: "Compliance",
    items: [
      { href: "/annix-sentinel/documents", label: "Documents", icon: FileText },
      { href: "/annix-sentinel/templates", label: "Templates", icon: FileStack },
      { href: "/annix-sentinel/tender", label: "Tender Pack", icon: FileCheck },
      { href: "/annix-sentinel/health-report", label: "Health Report", icon: HeartPulse },
    ],
  },
  {
    label: "Tools",
    items: [
      { href: "/annix-sentinel/bbee", label: "B-BBEE Tools", icon: Award },
      { href: "/annix-sentinel/tax-tools", label: "Tax Tools", icon: Calculator },
      { href: "/annix-sentinel/tax-calendar", label: "Tax Calendar", icon: CalendarDays },
      { href: "/annix-sentinel/seta-grants", label: "SETA Grants", icon: GraduationCap },
      { href: "/annix-sentinel/hr-tools", label: "HR Tools", icon: Users },
      { href: "/annix-sentinel/ai-assistant", label: "AI Assistant", icon: MessageSquare },
    ],
  },
  {
    label: "Advisor",
    items: [
      { href: "/annix-sentinel/advisor", label: "Client Overview", icon: Users },
      { href: "/annix-sentinel/advisor/calendar", label: "Deadline Calendar", icon: CalendarDays },
    ],
  },
  {
    label: "Account",
    items: [
      { href: "/annix-sentinel/notifications", label: "Notifications", icon: Bell },
      { href: "/annix-sentinel/settings", label: "Settings", icon: Settings },
      { href: "/annix-sentinel/api-keys", label: "API Keys", icon: Key },
      { href: "/annix-sentinel/integrations", label: "Integrations", icon: Plug },
    ],
  },
];
...
function isRouteActive(pathname: string, href: string): boolean {
  return (
    pathname === href || (href !== "/annix-sentinel/dashboard" && pathname.startsWith(`${href}/`))
  );
}
...
export default function AppShell({ children }: { children: React.ReactNode }) {
  ...
  function handleLogout() {
    ...
    router.push("/annix-sentinel/auth/login");
  }
  ...
  return (
    <div className="min-h-screen bg-slate-100 dark:bg-slate-900">
      ...
      <FeedbackWidget authContext="annix-sentinel" />
    </div>
  );
}
