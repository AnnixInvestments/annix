...
export function useNixAssistant({
  ...
  const [nixProcessingTimeRemaining, setNixProcessingTimeRemaining] = useState<number | undefined>(
    undefined,
  );
  ...
  const convertNixItemsToRfqItems = useCallback(
    (nixItems: NixExtractedItem[]) => {
  ...
  const handleProcessDocumentsWithNix = useCallback(async () => {
    ...
    setCurrentStep,
    showToast,
    convertNixItemsToRfqItems,
  ]);

  const handleClarificationSubmit = useCallback(
    async (clarificationId: number, response: string) => {
