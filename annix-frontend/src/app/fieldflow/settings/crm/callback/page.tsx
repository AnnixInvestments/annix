...
import type { CrmProvider } from "@/app/lib/api/annixRepApi";
import { useCrmOAuthCallback } from "@/app/lib/query/hooks";

type ConnectionState = "connecting" | "success" | "error";

function CrmOAuthCallbackContent() {
  const searchParams = useSearchParams();
  const router = useRouter();
  const [state, setState] = useState<ConnectionState>("connecting");
  const [errorMessage, setErrorMessage] = useState<string | null>(null);
  const oauthCallback = useCrmOAuthCallback();
  ...
  useEffect(() => {
    ...
    try {
      ...
      provider = decodedState.provider as CrmProvider;
    } catch {
      setState("error");
    ...
    if (!provider) {
      setState("error");
      setErrorMessage("Missing CRM provider in state");
      return;
    }
    ...
    oauthCallback.mutate(
      ...
      },
      {
        onSuccess: () => {
          setState("success");
          if (window.opener) {
            window.opener.postMessage({ type: "CRM_CONNECTED", provider }, window.location.origin);
            window.close();
          }
        },
        onError: (err) => {
          setState("error");
          setErrorMessage(err instanceof Error ? err.message : "Failed to connect CRM");
        },
      },
    );
          ...
          {state === "connecting" && (
            <>
              ...
              <p className="text-gray-500 dark:text-gray-400">
                Please wait while we connect your CRM...
              </p>
            </>
          )}
          ...
          {state === "success" && (
            <>
              ...
              <p className="text-gray-500 dark:text-gray-400 mb-4">
                Your CRM has been successfully connected. You can close this window.
              </p>
              <button
                onClick={handleClose}
