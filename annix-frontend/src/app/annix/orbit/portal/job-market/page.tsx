...
export default function JobMarketPage() {
  ...
  const { data: stats } = useOrbitJobMarketStats();
  const { data: sources = [] } = useOrbitJobMarketSources();
  const { data: jobsData, isLoading } = useOrbitExternalJobs({
  ...
  const rawTotal = jobsData?.total;
  const jobs = rawJobs || [];
  const jobsTotal = rawTotal || 0;
  const totalPages = Math.ceil(jobsTotal / 20);

  const triggerIngestion = useOrbitTriggerIngestion();
  const updateSource = useOrbitUpdateJobMarketSource();
  const deleteSource = useOrbitDeleteJobMarketSource();
  const createSource = useOrbitCreateJobMarketSource();
