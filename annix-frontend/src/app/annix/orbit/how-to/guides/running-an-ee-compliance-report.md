...
readingMinutes: 5
relatedPaths: [annix-frontend/src/app/annix/orbit/portal/compliance/ee-report, annix-backend/src/annix-orbit/services/ee-report.service.ts, annix-backend/src/annix-orbit/controllers/compliance.controller.ts]
---
...
The page is at `/annix/orbit/portal/compliance/ee-report` and is gated
...
straight to `/annix/orbit/portal/compliance/ee-report`.
...
The page fires `GET /annix/orbit/compliance/ee-report?dateFrom=…&dateTo=…`
which:
...
- **Download CSV** — opens a multi-section CSV (one per table from
  step 4) at `/annix/orbit/compliance/ee-report.csv?dateFrom=…&dateTo=…`.
...
- **Download PDF** — opens a single-page A4 PDF at
  `/annix/orbit/compliance/ee-report.pdf?dateFrom=…&dateTo=…`. Use
...
Today this is HR-API only — `POST /annix/orbit/candidates/:id/ee-disclosure-invite`.
The candidate gets an email with a tokenised link to
`/annix/orbit/ee-disclosure/[token]`, fills the form (all fields
