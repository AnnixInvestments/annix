...
export default function SageExportModal(props: SageExportModalProps) {
  ...
  const fetchPreview = useCallback(async () => {
    try {
      ...
      setError(null);
      const result = await auRubberApiClient.sageExportPreview({
  ...
  const handleExport = async () => {
    try {
      setIsExporting(true);
      setError(null);
      const blob = await auRubberApiClient.sageExportCsv({
        dateFrom: dateFrom || undefined,
        dateTo: dateTo || undefined,
        excludeExported,
      });
      ...
      <div className="bg-white rounded-lg shadow-xl max-w-md w-full mx-4">
        <div className="px-6 py-4 border-b border-gray-200">
          ...
          <p className="mt-1 text-sm text-gray-500">
            Export approved supplier invoices as CSV for Sage Business Cloud import
          </p>
        </div>
