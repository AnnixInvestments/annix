...
export default function AuRubberProductsPage() {
  ...
  const handleBulkDelete = async () => {
    ...
    const results = await Promise.allSettled(ids.map((id) => auRubberApiClient.deleteProduct(id)));
    const successCount = results.filter((r) => r.status === "fulfilled").length;
    const rejected = results.filter((r): r is PromiseRejectedResult => r.status === "rejected");
    const failCount = rejected.length;
    rejected.forEach((r) => console.error("Failed to delete product:", r.reason));
                ...
                <thead className="bg-gray-50">
                  <tr>
                    ...
                    >
                      Sp. Gravity
                      <SortIcon
                        active={sortColumn === "specificGravity"}
