...
export default function SupplierDeliveryNotesPage() {
  ...
  const handleFilesSelected = async (files: File[]) => {
    ...
    try {
      ...
      if (failed.size > 0) {
        alert({
          message:
            failed.size >= newDnIds.length
              ? "Extraction failed — the AI service (Gemini) may be temporarily overloaded. Please wait a few minutes and try again."
              : `${failed.size} of ${newDnIds.length} delivery notes could not be extracted (the AI service may be busy). Please re-upload those shortly.`,
          variant: "error",
        });
      } else {
  ...
  const noteRollNumbers = (note: RubberDeliveryNoteDto): string[] => {
    ...
    const edRolls = ed?.rolls;
    return (edRolls || []).map((r) => r.rollNumber).filter(Boolean);
  };

  if (error) {
    return (
      <BrandedErrorScreen
        area="Supplier Delivery Notes"
        error={error}
        ...
        <div className="flex space-x-3">
          ...
          <button
            onClick={async () => {
              const confirmed = await confirmDialog({
                ...
                message:
                  "Re-slices every supplier AND customer delivery note whose scanned PDF still has extra pages, so each note shows only the page(s) that mention its own DN number. Safe to run repeatedly; single-page documents are left untouched. This can take a couple of minutes.",
          ...
          <button
            onClick={async () => {
              ...
              showExtraction({
                brand: "au-rubber",
                label: `Re-extracting ${queueDepth} supplier DNs (background)…`,
              ...
              try {
                ...
                await waitForReExtractionComplete({
                  ...
                  onProgress: (done) => {
                    updateExtraction({
                      label: `Re-extracted ${done} of ${result.triggered} supplier DNs…`,
                    });
                  },
                });
                notesQuery.refetch();
