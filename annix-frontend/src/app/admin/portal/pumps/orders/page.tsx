...
export default function PumpOrdersPage() {
  ...
  const statusCounts = useMemo(() => {
    return Object.keys(STATUS_LABELS).reduce((acc, status) => {
      acc[status as MockPumpOrder['status']] = MOCK_ORDERS.filter((o) => o.status === status).length;
      return acc;
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
        {Object.entries(STATUS_LABELS).map(([status, label]) => (
          ...
          >
            ...
            <div className={`text-xs font-medium ${STATUS_COLORS[status as MockPumpOrder['status']]} px-2 py-0.5 rounded-full inline-block mt-1`}>
              {label}
            </div>
          </button>
        ))}
