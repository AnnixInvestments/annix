...
interface PvcFittingDimensionRow {
  id: number;
  fittingType: string;
  mainDnMm: number;
...
const FITTING_TYPE_LABELS: Record<string, string> = {
  elbow_11_25: "11.25° Elbow",
  elbow_22_5: "22.5° Elbow",
  elbow_45: "45° Elbow",
  elbow_90: "90° Elbow",
  tee_equal: "Equal Tee",
  ...
  end_cap: "End Cap",
  coupling_slip: "Slip Coupling (solvent-weld)",
  coupling_rrj: "RRJ Coupling (rubber-ring)",
  coupling_compression: "Compression / Repair Coupling",
  saddle: "Saddle",
  flange_adapter: "Flange Adapter (PVC stub)",
};
...
export default function PvcFittingDimensionsAdminPage() {
  const [rows, setRows] = useState<PvcFittingDimensionRow[]>([]);
  ...
  useEffect(() => {
    const fetchRows = async () => {
      ...
      setError(null);
      try {
        const response = await fetch(`${API_BASE_URL}/pvc-fitting-dimensions`);
        if (!response.ok) {
          throw new Error(`HTTP ${response.status}`);
        }
        const data: PvcFittingDimensionRow[] = await response.json();
        setRows(data);
      } catch (err) {
        const message = err instanceof Error ? err.message : "Unknown error";
        log.error("Failed to load PVC fitting dimensions:", err);
        setError(message);
      } finally {
        setLoading(false);
      }
  ...
  const groupedByType = useMemo(() => {
    const map = new Map<string, PvcFittingDimensionRow[]>();
    ...
    <div className="p-6 max-w-7xl mx-auto">
      <header className="mb-6">
        ...
        <p className="text-sm text-gray-600 dark:text-gray-400 leading-relaxed">
          SANS 1601 injection-moulded PVC fitting body geometry consumed by the BOQ row builder and
          the RFQ fitting picker. Source-of-truth lives in
          <code className="mx-1 px-1.5 py-0.5 bg-gray-100 dark:bg-gray-800 rounded text-xs">
            packages/product-data/pvc/*-dimensions.ts
          </code>
          and is seeded into this DB table on migration. Catalogue values come from manufacturer
          brochures (Marley, Macneil, Flo-Tek, DPI Trading) — never from SANS tables verbatim.
          Read-only in v1.
        </p>
      </header>
