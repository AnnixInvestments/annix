...
export default function AuMarketingBlogEditorPage() {
  ...
  const postQuery = useAuCmsBlogPost(isNew ? "" : id);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (isNew) {
        ...
        const created = await auCmsAdminApi.createBlogPost(dto);
        queryClient.invalidateQueries({ queryKey: auCmsKeys.blogPosts.all });
        showToast("Blog post created", "success");
        router.push(`/admin/portal/marketing/au/blog/${created.id}`);
      } else {
        ...
        await auCmsAdminApi.updateBlogPost(id, dto);
        queryClient.invalidateQueries({ queryKey: auCmsKeys.blogPosts.all });
