...
import {
  ...
  type ImportProductRowResultDto,
...
function parseNumericField(val: string | undefined, fieldName: string): { value?: number; error?: string } {
  if (!val?.trim()) return {};
...
function parseCSV(text: string): ParsedRow[] {
  ...
  const columnMap = headers.reduce((map, header, index) => {
    ...
    const matchedColumn = CSV_COLUMNS.find(
      (col) => col.toLowerCase() === normalizedHeader || col.toLowerCase().replace(/[_\s]/g, '') === normalizedHeader
    );
    if (matchedColumn) {
  ...
  return lines.slice(1).filter((line) => line.trim()).map((line, index) => {
    ...
    const specificGravityResult = parseNumericField(values[columnMap.specificGravity], 'specificGravity');
    ...
    return {
      ...
      title: columnMap.title !== undefined ? values[columnMap.title]?.trim() || undefined : undefined,
      description: columnMap.description !== undefined ? values[columnMap.description]?.trim() || undefined : undefined,
      type: columnMap.type !== undefined ? values[columnMap.type]?.trim() || undefined : undefined,
      compound: columnMap.compound !== undefined ? values[columnMap.compound]?.trim() || undefined : undefined,
      colour: columnMap.colour !== undefined ? values[columnMap.colour]?.trim() || undefined : undefined,
      hardness: columnMap.hardness !== undefined ? values[columnMap.hardness]?.trim() || undefined : undefined,
      grade: columnMap.grade !== undefined ? values[columnMap.grade]?.trim() || undefined : undefined,
      curingMethod: columnMap.curingMethod !== undefined ? values[columnMap.curingMethod]?.trim() || undefined : undefined,
      compoundOwner: columnMap.compoundOwner !== undefined ? values[columnMap.compoundOwner]?.trim() || undefined : undefined,
      firebaseUid: columnMap.firebaseUid !== undefined ? values[columnMap.firebaseUid]?.trim() || undefined : undefined,
  ...
  const handleFileChange = useCallback((e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (event) => {
      ...
      try {
        ...
        if (rows.length === 0) {
          setError('No valid rows found in CSV file. Make sure the file has a header row and at least one data row.');
          return;
        }
        setParsedRows(rows);
      } catch (err) {
        ...
        <div className="relative bg-white rounded-lg shadow-xl max-w-5xl w-full max-h-[90vh] overflow-hidden">
              ...
              <>
                <div className="mb-6">
                  ...
                  <p className="mt-2 text-xs text-gray-500">
                    Expected columns: title, description, type, compound, colour, hardness, grade, curingMethod, compoundOwner, specificGravity, costPerKg, markup, firebaseUid
                  </p>
                </div>
                        ...
                        <table className="min-w-full divide-y divide-gray-200 text-sm">
                          ...
                          <tbody className="bg-white divide-y divide-gray-200">
                            {parsedRows.slice(0, 100).map((row) => (
                              <tr key={row._rowIndex} className={row._parseErrors.length > 0 ? 'bg-amber-50' : ''}>
          ...
          <div className="px-6 py-4 border-t border-gray-200 flex justify-end space-x-3">
            ...
            {!importResult && parsedRows.length > 0 && (
              ...
              >
                {isImporting ? 'Importing...' : `Import ${parsedRows.length - rowsWithParseErrors.length} Products`}
              </button>
            )}
          </div>
        </div>
