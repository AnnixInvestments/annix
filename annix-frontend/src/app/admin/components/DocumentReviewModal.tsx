...
export function DocumentReviewModal({
  ...
  const renderDocumentPreview = () => {
    ...
    if (data.mimeType?.startsWith("image/")) {
      return (
        <img src={data.presignedUrl} alt={data.fileName} className="max-w-full object-contain" />
      );
    }
    ...
    if (data.mimeType === "application/pdf") {
      ...
      if (previewImages && previewImages.length > 0) {
        return <ImagePreviewViewer pages={previewImages} presignedUrl={data.presignedUrl} />;
      }
        ...
        <div className="relative bg-white rounded-lg shadow-xl max-w-7xl w-full max-h-[95vh] flex flex-col">
          ...
          <div className="flex-1 overflow-hidden flex">
            {isLoading ? (
              ...
              <>
                <div
                  className={`${isLikelyXfaPdf ? "w-2/5" : "w-1/2"} border-r overflow-auto bg-gray-100 p-4 flex flex-col`}
                >
                  ...
                  <div className="space-y-4">
                        ...
                        <div className="flex items-start gap-3">
                          ...
                          <div>
                            ...
                            <p className="text-sm text-blue-700 mt-1">
                              This appears to be a dynamic PDF that could not be automatically
                              processed. Please download the document using the button on the left,
                              open it in Adobe Reader, and manually verify the information matches
                    ...
                    {data.reviewedBy && (
                      <div className="text-sm text-gray-500">
                        Reviewed by {data.reviewedBy} on{" "}
                        {data.reviewedAt ? formatDateTimeZA(data.reviewedAt) : "N/A"}
                      </div>
                    )}
                  </div>
                </div>
