...
function valuesMatch(extracted: string | null, expected: string | number | null | undefined): boolean {
...
const fieldsPerDocumentType: Record<string, string[]> = {
  ...
  registration: ['companyName', 'registrationNumber', 'streetAddress', 'city', 'provinceState', 'postalCode'],
  registration_cert: ['companyName', 'registrationNumber', 'streetAddress', 'city', 'provinceState', 'postalCode'],
...
export function NixTrainingModal({
  ...
  const relevantFieldNames = fieldsPerDocumentType[documentType] || fieldsPerDocumentType['registration'] || [];
  const allFieldNames = [...relevantFieldNames, ...customFields];

  const fieldsToTrain: FieldComparisonResult[] = allFieldNames.length > 0
  ...
  const loadDocumentPages = useCallback(async () => {
    ...
    try {
      ...
      if (response.pages.length === 0) {
        setError('Unable to convert this PDF for training. The document may have unsupported graphics or formatting.');
      } else {
        setPages(response.pages);
      }
    } catch (err) {
      const message = err instanceof Error ? err.message : 'Failed to load document';
      setError(`Unable to load document for training: ${message}. Try using the document preview instead.`);
    } finally {
      setLoading(false);
    }
  ...
  }, [currentPage, scale, labelRegion, valueRegion, drawingPhase, drawingState, pages, trainedRegions]);
                ...
                <div className="flex-1 flex items-center justify-center p-6">
                  <div className="text-center max-w-md">
                    ...
                    <p className="text-xs text-gray-500 mb-4">
                      Some PDFs with complex graphics cannot be processed for training.
                      You can still manually enter field values using the &quot;Add Additional Field&quot; button.
                    </p>
                    <button
                ...
                <>
                  <div className="flex items-center justify-between bg-gray-800 text-white px-3 py-2">
                    ...
                    {selectedField && drawingPhase && (
                      <div className={`text-sm px-3 py-1 rounded ${drawingPhase === 'label' ? 'bg-purple-500' : 'bg-green-500'}`}>
                        Draw {drawingPhase === 'label' ? 'LABEL' : 'VALUE'} for: {fieldLabels[selectedField] || selectedField}
                      </div>
                    )}
                  </div>
            ...
            <div className="w-1/3 flex flex-col overflow-hidden">
              <div className="p-4 border-b bg-gray-50">
                ...
                <p className="text-sm text-gray-600">
                  Click a field, then draw two boxes: <span className="text-purple-600 font-medium">LABEL</span> (optional) and <span className="text-green-600 font-medium">VALUE</span>.
                </p>
              </div>
              ...
              <div className="flex-1 overflow-auto p-4">
                <div className="space-y-2">
                  {fieldsToTrain.map((field) => {
                    ...
                    const trainedMatchesExpected = trained?.saved && valuesMatch(trained.extractedText, field.expected);
                      ...
                      >
                        <div className="flex items-center justify-between">
                          <span className={`font-medium ${
                            ...
                            {isCustomField && (
                              <span className="ml-2 text-xs text-indigo-500 font-normal">(custom)</span>
                            )}
                          </span>
                          {trained?.saved ? (
                            trainedMatchesExpected ? (
                              <span className="text-green-600" title="Extracted value matches expected">
                                <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 20 20">
                                  <path fillRule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clipRule="evenodd" />
                                </svg>
                              </span>
                            ) : (
                              <span className="text-amber-600" title="Extracted value differs from expected - document may have different data">
                                <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 20 20">
                        ...
                        <div className="mt-1 text-sm">
                          {!isCustomField && (
                            <>
                              <div className={trained?.saved && !trainedMatchesExpected ? 'text-amber-700' : 'text-gray-500'}>
                          ...
                          {trained?.saved && (
                            <div className={`mt-1 ${trainedMatchesExpected || isCustomField ? 'text-green-600' : 'text-amber-600'}`}>
                              <div>Trained: {trained.extractedText} ({Math.round(trained.confidence * 100)}%)</div>
                  ...
                  {showAddField ? (
                    <div className="p-3 border border-indigo-300 bg-indigo-50 rounded-lg">
                      <div className="text-sm font-medium text-indigo-900 mb-2">Add Custom Field</div>
                      <input
                        type="text"
                        value={newFieldName}
                ...
                <div className="p-4 bg-gray-50 border-t">
                    ...
                    <div className="space-y-3">
                      <div className="p-3 bg-purple-50 border border-purple-200 rounded-lg">
                        ...
                        <p className="text-xs text-purple-600 mt-1">
                          Draw a box around the field label (e.g., &quot;Tax Number:&quot;, &quot;VAT No:&quot;)
                        </p>
                      </div>
                      <button
                  ...
                  {drawingPhase === 'label' && labelRegion && (
                    <div className="space-y-3">
                      ...
                      <p className="text-xs text-gray-500">Click &quot;Next&quot; to draw the value box, or redraw the label.</p>
                ...
                {(() => {
                  ...
                  if (trainedCount === totalFields) {
                    if (mismatchCount > 0) {
                      return (
                        <span>
                          All fields trained! <span className="text-green-600">{matchCount} matched</span>
                          {mismatchCount > 0 && (
                            <>, <span className="text-amber-600">{mismatchCount} with data discrepancies</span></>
                          )}
