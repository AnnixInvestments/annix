...
export function OperatingRangeValidator({
  ...
  const validation = useMemo((): ValidationResult => {
    ...
    for (let i = 0; i < sortedPoints.length - 1; i++) {
      if (operatingFlowM3h >= sortedPoints[i].flowM3h && operatingFlowM3h <= sortedPoints[i + 1].flowM3h) {
        const t = (operatingFlowM3h - sortedPoints[i].flowM3h) / (sortedPoints[i + 1].flowM3h - sortedPoints[i].flowM3h);
    ...
    if (npshMargin !== undefined) {
      if (npshMargin < 0) {
        issues.push({
          ...
          recommendation: 'Increase suction pressure, reduce friction losses, or lower pump elevation',
        });
    ...
    const headPoints = sortedPoints.filter(p =>
      p.flowM3h >= operatingFlowM3h * 0.95 && p.flowM3h <= operatingFlowM3h * 1.05
    );
  ...
  const statusConfig = {
    optimal: { bg: 'bg-green-50', border: 'border-green-200', text: 'text-green-800', icon: 'text-green-500', label: 'Optimal' },
    acceptable: { bg: 'bg-blue-50', border: 'border-blue-200', text: 'text-blue-800', icon: 'text-blue-500', label: 'Acceptable' },
    marginal: { bg: 'bg-yellow-50', border: 'border-yellow-200', text: 'text-yellow-800', icon: 'text-yellow-500', label: 'Marginal' },
    unacceptable: { bg: 'bg-red-50', border: 'border-red-200', text: 'text-red-800', icon: 'text-red-500', label: 'Unacceptable' },
  };
        ...
        <div className={`${config.bg} ${config.border} border rounded-lg p-4`}>
          <div className="flex items-center gap-3">
            {validation.overallStatus === 'optimal' && (
              <svg className={`w-8 h-8 ${config.icon}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div>
              ...
              <p className={`text-sm ${config.text}`}>
                {validation.overallStatus === 'optimal' && 'Operating within preferred range near BEP'}
                ...
                {validation.overallStatus === 'unacceptable' && 'Operating conditions will damage pump'}
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          ...
          {validation.metrics.npshMargin !== undefined && (
            <div className={`rounded-lg p-3 text-center ${
              validation.metrics.npshMargin < 0 ? 'bg-red-50' :
              validation.metrics.npshMargin < 0.5 ? 'bg-yellow-50' : 'bg-green-50'
            }`}>
              <div className="text-xs text-gray-500 mb-1">NPSH Margin</div>
              <div className={`text-xl font-bold ${
                validation.metrics.npshMargin < 0 ? 'text-red-700' :
                validation.metrics.npshMargin < 0.5 ? 'text-yellow-700' : 'text-green-700'
              }`}>
                ...
                <div key={idx} className={`${sc.bg} ${sc.border} border rounded-lg p-3`}>
                  <div className="flex items-start gap-3">
                    {issue.severity === 'error' && (
                      <svg className={`w-5 h-5 ${sc.icon} flex-shrink-0 mt-0.5`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
