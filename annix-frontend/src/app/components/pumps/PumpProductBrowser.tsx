...
import { SelectionResult } from '@/app/lib/config/pumps/pumpSelectionGuide';
...
export function PumpProductBrowser({
  ...
  const filteredProducts = useMemo(() => {
    ...
    if (searchQuery) {
      ...
      result = result.filter(
        (p) =>
          ...
          (p.pumpType && p.pumpType.toLowerCase().includes(query))
      );
    }
    ...
    if (applicationFilter) {
      result = result.filter((p) => p.pumpType?.includes(applicationFilter) || p.description?.includes(applicationFilter));
    }
    ...
    return result;
  }, [products, searchQuery, categoryFilter, manufacturerFilter, applicationFilter, flowRateRange, headRange]);
  ...
  const handleApi610Complete = useCallback((result: any, criteria: any) => {
    ...
    if (topType) {
      const category = topType.type.category === 'OH' ? 'centrifugal' :
        topType.type.category === 'BB' ? 'centrifugal' : 'specialty';
          ...
          <div className="flex gap-2">
            ...
            <button
              ...
              className={`px-3 py-2 border rounded-md flex items-center gap-2 ${
                showAdvancedFilters ? 'border-blue-500 text-blue-600' : 'border-gray-300 text-gray-600'
              }`}
            >
              <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div>
              ...
              <div className="flex gap-2">
                <input
                  ...
                  onChange={(e) => setFlowRateRange((prev) => ({ ...prev, min: e.target.value ? parseFloat(e.target.value) : undefined }))}
                  placeholder="Min"
                ...
                <input
                  ...
                  onChange={(e) => setFlowRateRange((prev) => ({ ...prev, max: e.target.value ? parseFloat(e.target.value) : undefined }))}
                  placeholder="Max"
                  className="w-1/2 border border-gray-300 rounded-md px-2 py-2"
                />
              </div>
            ...
            <div>
              ...
              <div className="flex gap-2">
                <input
                  type="number"
                  value={headRange.min ?? ''}
                  onChange={(e) => setHeadRange((prev) => ({ ...prev, min: e.target.value ? parseFloat(e.target.value) : undefined }))}
                  placeholder="Min"
                  className="w-1/2 border border-gray-300 rounded-md px-2 py-2"
                />
                <input
                  type="number"
                  value={headRange.max ?? ''}
                  onChange={(e) => setHeadRange((prev) => ({ ...prev, max: e.target.value ? parseFloat(e.target.value) : undefined }))}
                  placeholder="Max"
                  className="w-1/2 border border-gray-300 rounded-md px-2 py-2"
                />
              </div>
