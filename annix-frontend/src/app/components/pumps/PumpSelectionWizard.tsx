...
export function PumpSelectionWizard({ onComplete, onSelectPumpType }: PumpSelectionWizardProps) {
  ...
  const updateCriteria = <K extends keyof SelectionCriteria>(key: K, value: SelectionCriteria[K]) => {
    setCriteria((prev) => ({ ...prev, [key]: value }));
  };

  const togglePriority = (priority: 'efficiency' | 'cost' | 'reliability' | 'maintenance' | 'footprint') => {
    setCriteria((prev) => {
  ...
  const stepIndex = ['application', 'operating', 'fluid', 'priorities', 'results'].indexOf(currentStep);
            ...
            {['application', 'operating', 'fluid', 'priorities', 'results'].map((step, idx) => (
              <div
                ...
                className={`w-3 h-3 rounded-full ${
                  idx < stepIndex ? 'bg-green-500' : idx === stepIndex ? 'bg-blue-500' : 'bg-gray-300'
                }`}
              />
            ))}
          </div>
      ...
      <div className="p-6">
        {currentStep === 'application' && (
          <div className="space-y-4">
            ...
            <p className="text-sm text-gray-600">Choose the application that best matches your use case</p>
        ...
        {currentStep === 'operating' && (
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Operating Mode</label>
                <select
                  value={criteria.operatingMode ?? ''}
                  onChange={(e) => updateCriteria('operatingMode', e.target.value as SelectionCriteria['operatingMode'])}
                  className="w-full border border-gray-300 rounded-md px-3 py-2 focus:ring-blue-500 focus:border-blue-500"
                >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div>
                ...
                <select
                  ...
                  onChange={(e) => updateCriteria('fluidType', e.target.value as SelectionCriteria['fluidType'])}
                  className="w-full border border-gray-300 rounded-md px-3 py-2 focus:ring-blue-500 focus:border-blue-500"
                >
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Solids Content (%)</label>
                <input
                  type="number"
              ...
              {(criteria.solidsPercent ?? 0) > 0 && (
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">Max Particle Size (mm)</label>
                  <input
                    type="number"
                    value={criteria.particleSizeMm ?? 0}
                    onChange={(e) => updateCriteria('particleSizeMm', parseFloat(e.target.value) || 0)}
                    className="w-full border border-gray-300 rounded-md px-3 py-2 focus:ring-blue-500 focus:border-blue-500"
                    min={0}
                  />
                </div>
        ...
        {currentStep === 'priorities' && (
          <div className="space-y-4">
            ...
            <p className="text-sm text-gray-600">Select what matters most for your application (optional)</p>

            <div className="space-y-3">
              {PRIORITY_OPTIONS.map((priority) => {
                const priorityValue = priority.value as 'efficiency' | 'cost' | 'reliability' | 'maintenance' | 'footprint';
          ...
          <div className="space-y-6">
              ...
              {result.recommendedTypes.map((rec, idx) => (
                ...
                >
                  <div className="flex items-start justify-between">
                    <div>
                      <div className="flex items-center gap-2">
                        <span className={`text-lg font-semibold ${idx === 0 ? 'text-green-700' : 'text-gray-900'}`}>
                          {rec.type.label}
                        </span>
                        {idx === 0 && (
                        ...
                        <span className={`px-2 py-0.5 text-xs rounded-full ${
                          rec.type.category === 'centrifugal' ? 'bg-blue-100 text-blue-700' :
                          rec.type.category === 'positive_displacement' ? 'bg-purple-100 text-purple-700' :
                    ...
                    <div className="mt-3">
                      <div className="flex flex-wrap gap-2">
                        {rec.warnings.map((warning, i) => (
                          <span key={i} className="px-2 py-1 text-xs rounded bg-yellow-100 text-yellow-700">
                            {warning}
                          </span>
                        ))}
                      </div>
                  ...
                  <div className="mt-3 pt-3 border-t border-gray-200 grid grid-cols-2 md:grid-cols-4 gap-3 text-xs">
                    <div>
                      ...
                      <span className="ml-1 font-medium">{rec.type.efficiencyRange.min}-{rec.type.efficiencyRange.max}%</span>
                    </div>
                    <div>
            ...
            {result.considerations.length > 0 && (
              <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4">
                <h4 className="text-sm font-medium text-yellow-800 mb-2">Additional Considerations</h4>
