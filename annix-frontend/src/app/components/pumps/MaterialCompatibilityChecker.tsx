...
export function MaterialCompatibilityChecker({
  ...
  const result = useMemo((): CompatibilityResult | null => {
    ...
    const ratings = MATERIALS.map((mat) => evaluateMaterialCompatibility(fluid, mat)).sort(
      (a, b) => {
    ...
    <div className="bg-white rounded-lg shadow-sm border border-gray-200">
      ...
      <div className="p-4 space-y-4">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">Fluid Preset</label>
            <select
            ...
            >
              ...
              <optgroup label="Other">
                {FLUID_PRESETS.filter(
                  (p) => p.category === "solvent" || p.category === "other",
                ).map((p) => (
                  <option key={p.name} value={p.name}>
                    {p.name}
                  </option>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div>
            ...
            <input
              ...
              onChange={(e) =>
                setFluid((prev) => ({ ...prev, temperatureC: parseFloat(e.target.value) || 0 }))
              }
              className="w-full border border-gray-300 rounded-md px-3 py-2 focus:ring-blue-500 focus:border-blue-500"
            />
          </div>
          ...
          <div>
            ...
            <input
              ...
              onChange={(e) =>
                setFluid((prev) => ({
                  ...prev,
                  concentration: e.target.value ? parseFloat(e.target.value) : undefined,
                }))
              }
              className="w-full border border-gray-300 rounded-md px-3 py-2 focus:ring-blue-500 focus:border-blue-500"
        ...
        <div className="border-t border-gray-200 p-4 space-y-4">
                    ...
                    <ul className="mt-2 text-sm text-gray-600 space-y-1">
                      ...
                      {rating.maxTemperatureC !== undefined && (
                        <li className="text-gray-500">
                          Max recommended temp: {rating.maxTemperatureC}°C
                        </li>
                      )}
                    </ul>
                  </div>
          ...
          <div className="bg-gray-50 rounded-lg p-4">
            ...
            <p className="text-xs text-gray-500">
              ...
              metallurgical experts. Field conditions may vary significantly from general
              guidelines. Always consult material suppliers and relevant standards (NACE, ASTM, API)
              for final selection.
            </p>
          </div>
        </div>
