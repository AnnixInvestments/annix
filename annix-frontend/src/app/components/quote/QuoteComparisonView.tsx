...
export default function QuoteComparisonView({
  ...
  const sortedQuotes = useMemo(() => {
    ...
    if (sortBy === "price") {
      quoted.sort(
        (a, b) =>
          (a.quoteData?.totals?.grandTotalIncVat || 0) -
          (b.quoteData?.totals?.grandTotalIncVat || 0),
      );
    } else if (sortBy === "supplier") {
      quoted.sort((a, b) =>
        (a.companyName || a.supplierName).localeCompare(b.companyName || b.supplierName),
      );
    } else if (sortBy === "date") {
      quoted.sort((a, b) => {
  ...
  const lowestPrice = useMemo(() => {
    ...
    return Math.min(
      ...quotedSuppliers.map((q) => q.quoteData?.totals?.grandTotalIncVat || Infinity),
    );
  }, [quotedSuppliers]);
                ...
                return (
                  ...
                  >
                    ...
                    <td className="py-3 px-4 text-right">
                      <span
                        className={`font-bold ${isLowest ? "text-green-700" : "text-gray-900"}`}
                      >
        ...
        <div className="bg-white border border-gray-200 rounded-lg">
          <div className="px-4 py-3 border-b border-gray-200 bg-gray-50">
            <h3 className="font-semibold text-gray-700">
              Awaiting Response ({pendingSuppliers.length})
            </h3>
          </div>
          <div className="p-4">
            <div className="flex flex-wrap gap-3">
              {pendingSuppliers.map((supplier) => (
                ...
                >
                  <span
                    className={`w-2 h-2 rounded-full ${supplier.status === "viewed" ? "bg-blue-500" : "bg-yellow-500"}`}
                  />
                  <span className="text-gray-700">
                    {supplier.companyName || supplier.supplierName}
                  </span>
                  <span
                    className={`text-xs px-1.5 py-0.5 rounded ${STATUS_BADGES[supplier.status].bg} ${STATUS_BADGES[supplier.status].text}`}
                  >
      ...
      {declinedSuppliers.length > 0 && (
        <div className="bg-white border border-gray-200 rounded-lg">
          ...
          >
            ...
            <svg
              className={`w-5 h-5 text-gray-500 transition-transform ${showDeclined ? "rotate-180" : ""}`}
              fill="none"
              stroke="currentColor"
              viewBox="0 0 24 24"
            >
