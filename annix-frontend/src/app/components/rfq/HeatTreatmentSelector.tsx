...
import type { HeatTreatment, HeatTreatmentRequirementResponse } from '@/app/lib/pipe-steel-work/types';
...
export default function HeatTreatmentSelector({
  ...
  const { treatments, isLoading, treatmentByCode, checkRequirement, isCheckingRequirement } = useHeatTreatments();
  ...
  const selectedTreatment = useMemo(() => (value ? treatmentByCode(value) : null), [value, treatmentByCode]);
  ...
  const filteredTreatments = useMemo(() => {
    ...
    return treatments.filter((t) => t.applicableMaterials.some((m) => material.toUpperCase().includes(m.toUpperCase())));
  }, [treatments, material]);
    ...
    const runCheck = async () => {
      ...
      if (!abortController.signal.aborted) {
        ...
        if (result?.isRequired && result.requiredTreatment && !value) {
          const requiredTreatmentCode = treatments.find((t) => t.type === result.requiredTreatment)?.code;
  ...
  }, [material, wallThicknessMm, weldType, localWeldType, designCode, localDesignCode, showRequirementCheck, checkRequirement, treatments, value, onChange]);
        ...
        <div className="flex-1">
          ...
          >
            ...
            {Object.entries(treatmentsByType).map(([type, treats]) => {
              const filtered = treats.filter((t) =>
                !material || t.applicableMaterials.some((m) => material.toUpperCase().includes(m.toUpperCase()))
              );
        ...
        <div className="bg-orange-50 rounded-md p-3 text-sm border border-orange-200">
          ...
          <div className="grid grid-cols-2 gap-3 text-xs">
            ...
            <div>
              ...
              <p className="font-medium text-gray-700">{selectedTreatment.heatingRateMaxCPerHr}°C/hr</p>
            </div>
            <div>
          ...
          <div className="mt-2 text-xs">
            ...
            <span className="font-medium text-gray-700">{selectedTreatment.codeReferences.join(', ')}</span>
          </div>
        </div>
      )}
      ...
      {requirement && (
        <div
          className={`rounded-md p-3 ${
            requirement.isRequired ? 'bg-red-50 border border-red-200' : 'bg-gray-50 border border-gray-200'
          }`}
        >
          <div className="flex items-start gap-2">
            ...
            <div>
              <p className={`text-sm font-medium ${requirement.isRequired ? 'text-red-700' : 'text-green-700'}`}>
          ...
          {requirement.isRequired && requirement.estimatedCostImpact > 0 && (
            <div className="mt-2 pt-2 border-t border-gray-200">
              <p className="text-xs text-gray-600">
                ...
                <span className="text-orange-600 font-semibold">+R{requirement.estimatedCostImpact.toFixed(2)}</span>
              </p>
            </div>
          )}
        </div>
