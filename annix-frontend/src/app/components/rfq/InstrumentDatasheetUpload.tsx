...
interface InstrumentDatasheetUploadProps {
    ...
    <div className="space-y-2">
      ...
      {datasheets.length < maxDatasheets && (
        ...
        >
          <input
            type="file"
            id="instrument-datasheet-upload"
            multiple
            accept=".pdf,.jpg,.jpeg,.png"
            onChange={handleFileSelect}
            className="hidden"
          />
          <label htmlFor="instrument-datasheet-upload" className="cursor-pointer flex items-center justify-center gap-3">
            ...
            <div className="text-left">
              <p className="text-sm text-gray-600">
                <span className="font-medium text-cyan-600">Upload instrument datasheet</span>
                <span className="text-gray-400 ml-2">({datasheets.length}/{maxDatasheets})</span>
              </p>
              <p className="text-xs text-gray-400">Specs, wiring diagrams, calibration certs (PDF, images)</p>
            </div>
          </label>
        </div>
      )}
      ...
      {datasheets.length > 0 && (
        <div className="flex flex-wrap gap-2">
          {datasheets.map((doc) => (
            <div
              key={doc.id}
              className="flex items-center gap-2 px-2 py-1.5 bg-cyan-50 rounded border border-cyan-200 text-sm"
            >
