...
export const LeaderLine = ({
  ...
  const shelfEnd = useMemo(
    () =>
      new THREE.Vector3(
        elbowPoint.x + direction * shelfLength,
        elbowPoint.y,
        elbowPoint.z
      ),
  ...
  return (
    <group>
      ...
      {arrowStyle === 'open' && <Line points={arrowPoints} color={color} lineWidth={dimLineWidth} />}
...
export const ChainDimension = ({
  ...
  const totalOffsetVector = useMemo(() => {
    if (offsetDirection instanceof THREE.Vector3) {
      return offsetDirection.clone().normalize().multiplyScalar(offset + totalOffset)
    }
    const vec = new THREE.Vector3()
  ...
  return (
    <group>
      {segments.map((seg, idx) => {
        const direction = new THREE.Vector3().subVectors(seg.endOffset, seg.startOffset).normalize()
        ...
        const rightArrowPoints = createArrowPoints(seg.endOffset, direction.clone().negate(), arrowSize)
        ...
        const extStartGap = seg.start.clone().add(extGapDir.multiplyScalar(DIMENSION_STANDARDS.extensionGap * Math.sign(offset)))
        const extEndGap = seg.end.clone().add(extGapDir.clone().multiplyScalar(DIMENSION_STANDARDS.extensionGap * Math.sign(offset)))
        const extStartOvershoot = seg.startOffset.clone().add(extGapDir.clone().multiplyScalar(DIMENSION_STANDARDS.extensionOvershoot * Math.sign(offset)))
        const extEndOvershoot = seg.endOffset.clone().add(extGapDir.clone().multiplyScalar(DIMENSION_STANDARDS.extensionOvershoot * Math.sign(offset)))
        ...
        return (
          <group key={idx}>
            ...
            {arrowStyle === 'filled' && (
              <>
                <mesh geometry={createFilledArrowGeometry(seg.startOffset, direction.clone().negate(), arrowSize)}>
                  <meshBasicMaterial color={color} side={THREE.DoubleSide} />
                </mesh>
            ...
            {useHtml ? (
              <Html position={[midPoint.x, midPoint.y + DIMENSION_STANDARDS.textOffset, midPoint.z]} center>
                <div
                  style={{
                    color,
                    fontSize: `${fontSize * 100}px`,
      ...
      {showTotal && segments.length > 0 && (
        <group>
          {(() => {
            ...
            const totalMidPoint = new THREE.Vector3().lerpVectors(totalStartOffset, totalEndOffset, 0.5)
            const totalDir = new THREE.Vector3().subVectors(totalEndOffset, totalStartOffset).normalize()
              ...
              <>
                ...
                {arrowStyle === 'open' && (
                  <>
                    ...
                    />
                    <Line
                      points={createArrowPoints(totalEndOffset, totalDir.clone().negate(), totalArrowSize)}
                      color={color}
                      lineWidth={dimLineWidth}
                    />
                  </>
                ...
                {useHtml ? (
                  <Html position={[totalMidPoint.x, totalMidPoint.y + DIMENSION_STANDARDS.textOffset, totalMidPoint.z]} center>
                    <div
                      style={{
                        color,
                        fontSize: `${fontSize * 100}px`,
                  ...
                  <Text
                    position={[totalMidPoint.x, totalMidPoint.y + DIMENSION_STANDARDS.textOffset, totalMidPoint.z]}
                    fontSize={fontSize}
                    color={color}
                    anchorX="center"
                    anchorY="bottom"
  ...
  return (
    <group>
      {sortedPoints.map((point, idx) => {
        ...
        const extBaselineGap = baseline.clone().add(extGapDir.multiplyScalar(DIMENSION_STANDARDS.extensionGap))
        const extPointGap = point.position.clone().add(extGapDir.clone().multiplyScalar(DIMENSION_STANDARDS.extensionGap))

        const maxOffset = baseOffset + (sortedPoints.length - 1) * spacingIncrement + DIMENSION_STANDARDS.extensionOvershoot
        const extBaselineOvershoot = baseline.clone().add(baseOffsetVector.clone().multiplyScalar(maxOffset))
        const extPointOvershoot = point.position.clone().add(baseOffsetVector.clone().multiplyScalar(offset + DIMENSION_STANDARDS.extensionOvershoot))
        ...
        return (
          <group key={idx}>
            ...
            {arrowStyle === 'filled' && (
              <>
                <mesh geometry={createFilledArrowGeometry(startOffset, direction.clone().negate(), arrowSize)}>
                  <meshBasicMaterial color={color} side={THREE.DoubleSide} />
                </mesh>
  ...
  return (
    <group>
      {spacedDimensions.map((dim, idx) => {
        ...
        const extStartGap = dim.start.clone().add(extGapDir.clone().multiplyScalar(DIMENSION_STANDARDS.extensionGap * sign))
        const extEndGap = dim.end.clone().add(extGapDir.clone().multiplyScalar(DIMENSION_STANDARDS.extensionGap * sign))
        const extStartOvershoot = startOffset.clone().add(extGapDir.clone().multiplyScalar(DIMENSION_STANDARDS.extensionOvershoot * sign))
        const extEndOvershoot = endOffset.clone().add(extGapDir.clone().multiplyScalar(DIMENSION_STANDARDS.extensionOvershoot * sign))
        ...
        return (
          <group key={idx}>
            ...
            {arrowStyle === 'tick' && (
              <>
                <Line points={createTickPoints(startOffset, perpDir, arrowSize)} color={color} lineWidth={dimLineWidth} />
                <Line points={createTickPoints(endOffset, perpDir, arrowSize)} color={color} lineWidth={dimLineWidth} />
              </>
            )}
