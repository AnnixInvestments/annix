...
function parseStubFromDescription(
  description: string,
): { nb: number; flangeConfig: string } | null {
  const stubMatch = description.match(/(\d+)NB\s*x\s*\d+mm\s*Stub\s*(\w+)/i);
  if (stubMatch) {
    return {
      nb: parseInt(stubMatch[1], 10),
...
function parseFlangeConfigFromDescription(
  description: string,
  itemType: string,
): {
  mainFlanges: number;
  ...
  if (itemType === "straight_pipe") {
    ...
    if (rfMatch) result.mainFlanges += parseInt(rfMatch[1], 10);
    if (lfMatch) result.mainFlanges += parseInt(lfMatch[1], 10);
...
function getBnwSetInfo(
...
): { boltSize: string; holesPerFlange: number; weightPerSet: number } {
  ...
  > = {
    50: {
      16: { boltSize: "M16", holes: 4, weight: 1.0 },
      25: { boltSize: "M16", holes: 4, weight: 1.2 },
      40: { boltSize: "M16", holes: 4, weight: 1.4 },
    },
    80: {
      16: { boltSize: "M16", holes: 8, weight: 2.0 },
      25: { boltSize: "M16", holes: 8, weight: 2.4 },
      40: { boltSize: "M20", holes: 8, weight: 4.0 },
    },
    100: {
      16: { boltSize: "M16", holes: 8, weight: 2.4 },
      25: { boltSize: "M20", holes: 8, weight: 4.0 },
      40: { boltSize: "M20", holes: 8, weight: 4.8 },
    },
    150: {
      16: { boltSize: "M20", holes: 8, weight: 4.0 },
      25: { boltSize: "M20", holes: 8, weight: 4.8 },
      40: { boltSize: "M24", holes: 8, weight: 7.2 },
    },
    200: {
      16: { boltSize: "M20", holes: 8, weight: 4.8 },
      25: { boltSize: "M20", holes: 12, weight: 8.4 },
      40: { boltSize: "M24", holes: 12, weight: 12.0 },
    },
    250: {
      16: { boltSize: "M20", holes: 12, weight: 8.4 },
      25: { boltSize: "M24", holes: 12, weight: 12.0 },
      40: { boltSize: "M27", holes: 12, weight: 15.6 },
    },
    300: {
      16: { boltSize: "M20", holes: 12, weight: 9.6 },
      25: { boltSize: "M24", holes: 12, weight: 13.2 },
      40: { boltSize: "M27", holes: 16, weight: 24.0 },
    },
    350: {
      16: { boltSize: "M20", holes: 12, weight: 10.8 },
      25: { boltSize: "M24", holes: 16, weight: 19.2 },
      40: { boltSize: "M30", holes: 16, weight: 28.8 },
    },
    400: {
      16: { boltSize: "M24", holes: 16, weight: 16.0 },
      25: { boltSize: "M27", holes: 16, weight: 22.4 },
      40: { boltSize: "M30", holes: 16, weight: 32.0 },
    },
    450: {
      16: { boltSize: "M24", holes: 16, weight: 17.6 },
      25: { boltSize: "M27", holes: 20, weight: 30.0 },
      40: { boltSize: "M33", holes: 20, weight: 46.0 },
    },
    500: {
      16: { boltSize: "M24", holes: 20, weight: 24.0 },
      25: { boltSize: "M30", holes: 20, weight: 34.0 },
      40: { boltSize: "M33", holes: 20, weight: 50.0 },
    },
    600: {
      16: { boltSize: "M27", holes: 20, weight: 28.0 },
      25: { boltSize: "M30", holes: 20, weight: 38.0 },
      40: { boltSize: "M36", holes: 20, weight: 60.0 },
    },
  };
...
async function fixBoqSections() {
  ...
  try {
    ...
    for (const item of rfqItems) {
      ...
      console.log(
        `     Main NB: ${mainNb}, Config: main=${flangeConfig.mainFlanges}, branch=${flangeConfig.branchFlanges}, blank=${flangeConfig.blankFlanges}`,
      );
      if (stubInfo) {
