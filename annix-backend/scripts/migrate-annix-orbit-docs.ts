 ...
 *   Old: uploads/annix-orbit/{companyId}/*  or  uploads/annix-orbit/manual/*
 *   New: annix-orbit/candidates/{companyId}/*
 *
 * It processes:
...
async function migrateAnnixOrbitDocs() {
  ...
  logger.log("Migrating from local uploads/ to S3 annix-orbit/candidates/");
  logger.log("=".repeat(60));
  ...
  try {
    ...
    const records: CandidateRecord[] = await dataSource.query(`
      ...
      WHERE cv_file_path IS NOT NULL
        AND cv_file_path NOT LIKE 'annix-orbit/%'
      ORDER BY id
    `);
    ...
    for (const record of records) {
      try {
        ...
        const newS3Path = `annix-orbit/candidates/${companyId}/${filename}`;
  ...
  if (isDryRun) {
    ...
    logger.log("  uploads/annix-orbit/");
  }
