...
export class GasketWeightController {
  ...
  @ApiQuery({ name: 'gasketType', description: 'Gasket type (ASBESTOS, GRAPHITE, PTFE, RUBBER)' })
  @ApiQuery({ name: 'nb', description: 'Nominal bore in mm' })
  getGasketWeight(
    @Query('gasketType') gasketType: string,
    @Query('nb') nb: string,
  ) {
  ...
  @Get('flange')
  @ApiOperation({ summary: 'Get flange weight for specific NB and pressure class' })
  @ApiQuery({ name: 'nb', description: 'Nominal bore in mm' })
  @ApiQuery({ name: 'pressureClass', description: 'Pressure class (e.g., PN16, Class 150)' })
  @ApiQuery({ name: 'flangeStandard', required: false, description: 'Flange standard code (optional)' })
  getFlangeWeight(
    @Query('nb') nb: string,
  ...
  @Get('blank-flange')
  @ApiOperation({ summary: 'Get blank flange weight for specific NB and pressure class' })
  @ApiQuery({ name: 'nb', description: 'Nominal bore in mm' })
  @ApiQuery({ name: 'pressureClass', description: 'Pressure class (e.g., PN16, Class 150)' })
  getBlankFlangeWeight(
    @Query('nb') nb: string,
    @Query('pressureClass') pressureClass: string,
  ) {
    const nbNum = parseInt(nb, 10);
  ...
  @Get('bolt-set')
  @ApiOperation({ summary: 'Get bolt set info and weight for specific NB and pressure class' })
  @ApiQuery({ name: 'nb', description: 'Nominal bore in mm' })
  @ApiQuery({ name: 'pressureClass', description: 'Pressure class (e.g., PN16, Class 150)' })
  @ApiQuery({ name: 'flangeStandard', required: false, description: 'Flange standard code (optional)' })
  getBoltSetInfo(
