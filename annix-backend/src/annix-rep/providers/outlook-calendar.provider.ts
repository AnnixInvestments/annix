...
export class OutlookCalendarProvider implements ICalendarProvider {
  ...
  async syncEvents(
    ...
    await calendarIds.reduce(async (accPromise, calendarId) => {
      await accPromise;
      ...
      allEvents.push(...effectiveResult.events);
      allDeletedIds.push(...effectiveResult.deletedEventIds);
      if (effectiveResult.nextSyncToken) {
        newDeltaLinks[calendarId] = effectiveResult.nextSyncToken;
      }
    }, Promise.resolve());
  ...
  private async syncCalendarEvents(
    ...
    do {
      ...
      if (data.value) {
        const deleted = data.value.filter(
          (item) => item.isCancelled || (item.showAs && item.showAs === "free" && !item.subject),
        );
