...
describe("TeamService", () => {
  ...
  beforeEach(async () => {
    mockTeamMemberRepo = {
      ...
      findByUserAnyOrganization: jest.fn(),
  ...
  describe("members", () => {
    it("should return all members for an organization", async () => {
      ...
      (mockTeamMemberRepo.findByOrganization as jest.Mock).mockResolvedValue(members);

      const result = await service.members(10);

      expect(result).toEqual(members);
      expect(mockTeamMemberRepo.findByOrganization).toHaveBeenCalledWith(10);
    });

    it("should return empty array when no members exist", async () => {
      (mockTeamMemberRepo.findByOrganization as jest.Mock).mockResolvedValue([]);
  ...
  describe("activeMembers", () => {
    it("should return only active members", async () => {
      ...
      (mockTeamMemberRepo.findActiveByOrganization as jest.Mock).mockResolvedValue(activeMembers);

      const result = await service.activeMembers(10);

      expect(result).toEqual(activeMembers);
      expect(mockTeamMemberRepo.findActiveByOrganization).toHaveBeenCalledWith(10);
    });
  });

  describe("memberById", () => {
    it("should return member when found", async () => {
      (mockTeamMemberRepo.findByIdWithRelations as jest.Mock).mockResolvedValue(baseMember);

      const result = await service.memberById(1);

      expect(result).toEqual(baseMember);
      expect(mockTeamMemberRepo.findByIdWithRelations).toHaveBeenCalledWith(1);
    });

    it("should return null when member not found", async () => {
      (mockTeamMemberRepo.findByIdWithRelations as jest.Mock).mockResolvedValue(null);
  ...
  describe("memberByUser", () => {
    it("should return member for given org and user", async () => {
      (mockTeamMemberRepo.findByOrganizationAndUser as jest.Mock).mockResolvedValue(baseMember);

      const result = await service.memberByUser(10, 100);

      expect(result).toEqual(baseMember);
      expect(mockTeamMemberRepo.findByOrganizationAndUser).toHaveBeenCalledWith(10, 100);
    });
  ...
  describe("memberByUserAnyOrg", () => {
    it("should return member across any organization", async () => {
      (mockTeamMemberRepo.findByUserAnyOrganization as jest.Mock).mockResolvedValue(baseMember);

      const result = await service.memberByUserAnyOrg(100);

      expect(result).toEqual(baseMember);
      expect(mockTeamMemberRepo.findByUserAnyOrganization).toHaveBeenCalledWith(100);
    });
  });
  ...
  describe("addMember", () => {
    it("should create and save a new member", async () => {
      ...
      (mockTeamMemberRepo.create as jest.Mock).mockResolvedValue(created);
  ...
  describe("removeMember", () => {
    it("should remove the member", async () => {
      (mockTeamMemberRepo.findByIdWithRelations as jest.Mock).mockResolvedValue(baseMember);
      (mockTeamMemberRepo.remove as jest.Mock).mockResolvedValue(undefined);
  ...
  describe("setReportsTo", () => {
    it("should set reportsToId and save", async () => {
      ...
      (mockTeamMemberRepo.findByIdWithRelations as jest.Mock).mockResolvedValueOnce(member);
      (mockTeamMemberRepo.findByOrganizationAndUser as jest.Mock).mockResolvedValueOnce(manager);
    ...
    it("should throw NotFoundException when manager not found", async () => {
      const member = { ...baseMember, organizationId: 10 };
      (mockTeamMemberRepo.findByIdWithRelations as jest.Mock).mockResolvedValueOnce(member);
      (mockTeamMemberRepo.findByOrganizationAndUser as jest.Mock).mockResolvedValueOnce(null);
    ...
    it("should throw ConflictException when member reports to themselves", async () => {
      ...
      (mockTeamMemberRepo.findByIdWithRelations as jest.Mock).mockResolvedValueOnce(member);
      (mockTeamMemberRepo.findByOrganizationAndUser as jest.Mock).mockResolvedValueOnce(
        selfManager,
      );
  ...
  describe("teamHierarchy", () => {
    it("should build hierarchy tree from flat members", async () => {
      ...
      (mockTeamMemberRepo.findByOrganization as jest.Mock).mockResolvedValue([admin, rep]);
    ...
    it("should return multiple top-level nodes when no reports-to", async () => {
      ...
      (mockTeamMemberRepo.findByOrganization as jest.Mock).mockResolvedValue([memberA, memberB]);
  ...
  describe("directReports", () => {
    it("should return direct reports for a manager", async () => {
      ...
      (mockTeamMemberRepo.findDirectReports as jest.Mock).mockResolvedValue(reports);

      const result = await service.directReports(10, 100);

      expect(result).toEqual(reports);
      expect(mockTeamMemberRepo.findDirectReports).toHaveBeenCalledWith(10, 100);
    });
  });
  ...
  describe("isUserInOrganization", () => {
    ...
    it("should return false when user is inactive", async () => {
      ...
      (mockTeamMemberRepo.findByOrganizationAndUser as jest.Mock).mockResolvedValue(inactive);
