...
export class BendCenterToFaceController {
  constructor(
    private readonly bendCenterToFaceService: BendCenterToFaceService,
  ) {}
  ...
  findByCriteria(
    ...
    return this.bendCenterToFaceService.findByCriteria(
      bendType,
      nominalBoreMm,
      degrees,
    );
  ...
  @ApiOperation({
    summary:
      'Calculate bend specifications including weight and weld requirements',
  })
  @ApiBody({
    schema: {
      type: 'object',
      properties: {
        ...
        scheduleNumber: {
          type: 'string',
          description: 'Schedule number or designation',
        },
        bendType: {
          type: 'string',
          description: 'Bend type (1.5D, 2D, 3D, 5D)',
        },
        bendDegrees: { type: 'number', description: 'Bend angle in degrees' },
        numberOfTangents: {
          type: 'number',
          description: 'Number of tangent pipes',
          default: 0,
        },
        tangentLengths: {
          type: 'array',
        ...
        quantity: {
          type: 'number',
          description: 'Number of bend assemblies',
          default: 1,
        },
  ...
  @ApiResponse({
    status: 200,
    description:
      'Bend calculations including weight, dimensions, and weld requirements',
    schema: {
      type: 'object',
      properties: {
        centerToFaceDimension: {
          type: 'number',
        ...
        totalBendWeight: {
          type: 'number',
          description: 'Total weight of bend components in kg',
        },
        totalTangentWeight: {
          type: 'number',
          description: 'Total weight of tangent pipes in kg',
        },
        totalSystemWeight: {
          type: 'number',
          description: 'Total system weight including flanges in kg',
        },
        numberOfFlanges: {
          type: 'number',
          description: 'Total number of flanges required',
        },
        numberOfFlangeWelds: {
          type: 'number',
  ...
  async calculateBendSpecifications(
    @Body()
    body: {
