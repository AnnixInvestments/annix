...
export class WeldJointEfficiencyController {
  constructor(private readonly weldJointEfficiencyService: WeldJointEfficiencyService) {}
  ...
  lookupJointEfficiency(
    ...
    return this.weldJointEfficiencyService.getJointEfficiencyB31_1(typeOfJoint, examination);
  }
  ...
  getQualityFactorsByMaterial(@Query("materialCategory") materialCategory: string) {
    if (!materialCategory) {
      return { error: "materialCategory query parameter is required" };
    }
    const result = this.weldJointEfficiencyService.getQualityFactorsByMaterial(materialCategory);
    if (!result) {
  ...
  private getPipeTypeDescription(pipeType: string, radiographed: boolean): string {
    switch (pipeType) {
      case "seamless":
