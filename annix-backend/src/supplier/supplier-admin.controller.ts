...
import { RejectSupplierDto, ReviewDocumentDto, SuspendSupplierDto } from "./dto";
...
export class SupplierAdminController {
  ...
  async getAllSuppliers(
    ...
    return this.supplierAdminService.getAllSuppliers(page || 1, limit || 20, status);
  }
  ...
  async startReview(@Param("id", ParseIntPipe) supplierId: number, @Req() req: Request) {
    const adminUserId = req["user"].userId;
    const clientIp = this.getClientIp(req);
  ...
  async approveOnboarding(@Param("id", ParseIntPipe) supplierId: number, @Req() req: Request) {
    const adminUserId = req["user"].userId;
    const clientIp = this.getClientIp(req);
  ...
  async reactivateSupplier(@Param("id", ParseIntPipe) supplierId: number, @Req() req: Request) {
    const adminUserId = req["user"].userId;
    const clientIp = this.getClientIp(req);
  ...
  async inviteSupplier(@Body() body: { email: string; message?: string }, @Req() req: Request) {
