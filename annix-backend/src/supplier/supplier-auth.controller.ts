...
export class SupplierAuthController {
  ...
  async register(@Body() dto: CreateSupplierRegistrationDto, @Req() req: Request) {
  ...
  @ApiOperation({
    summary: "Register a new supplier account with company details and documents",
  })
  ...
  @ApiBody({
    schema: {
      ...
      properties: {
        ...
        security: {
          type: "string",
          description: "JSON string of security data (email, password, deviceFingerprint)",
        },
        vatDocument: {
          type: "string",
          format: "binary",
  ...
  async refresh(
    ...
    const clientIp = this.getClientIp(req);
    return this.supplierAuthService.refreshSession(
      body.refreshToken,
