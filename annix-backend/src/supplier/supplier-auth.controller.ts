...
export class SupplierAuthController {
  ...
  @ApiOperation({
    summary:
      'Register a new supplier account with company details and documents',
  })
    ...
    schema: {
      ...
      properties: {
        ...
        security: {
          type: 'string',
          description:
            'JSON string of security data (email, password, deviceFingerprint)',
        },
        vatDocument: {
          type: 'string',
          format: 'binary',
  ...
  async resendVerification(
    ...
    const clientIp = this.getClientIp(req);
    return this.supplierAuthService.resendVerificationEmail(
      body.email,
      clientIp,
    );
  }
