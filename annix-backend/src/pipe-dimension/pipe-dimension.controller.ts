...
export class PipeDimensionController {
  ...
  async create(
    /*@Param('nominalId', ParseIntPipe) nominalId: number,*/ @Body()
    createPipeDimensionDto: CreatePipeDimensionDto,
  ) {
    return this.pipeDimensionService.create(
      /*nominalId,*/ createPipeDimensionDto,
    );
  }
  ...
  async update(
    ...
    @Body() updatePipeDimensionDto: UpdatePipeDimensionDto,
  ) {
  ...
  @Post('recommend')
  @ApiOperation({
    summary: 'Get recommended pipe specifications based on working conditions',
  })
  ...
  @ApiOperation({
    summary: 'Get higher schedule options than recommended minimum',
  })
  @ApiQuery({
    name: 'nominalBore',
    type: Number,
  ...
  @ApiQuery({
    ...
    type: Number,
    required: false,
    description: 'Temperature in °C (default: 20)',
  })
  @ApiQuery({
    name: 'steelSpecId',
    type: Number,
  ...
  @ApiResponse({
    status: 200,
    description: 'List of higher schedule options',
    type: [PipeDimension],
  })
  ...
  @ApiOperation({
    summary:
      'Get all possible schedule numbers and wall thickness for a given steel specification and nominal bore',
  })
  @ApiResponse({
    status: 200,
    description: 'List of pipe dimensions with schedules and wall thicknesses',
  })
  @ApiResponse({
    status: 404,
    description:
      'No pipe dimensions found for the given steel specification and nominal bore',
  })
  @ApiResponse({
    status: 400,
    description: 'Invalid Steel Specification ID or Nominal Bore ID',
  })
  async findAllBySpecAndNominal(
    ...
    return this.pipeDimensionService.findAllBySpecAndNominal(
      steelSpecId,
      nominalId,
    );
  }
