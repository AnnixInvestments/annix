...
export class PvcController {
  ...
  @ApiParam({
    name: 'code',
    description: 'Standard code (e.g., EN_1452, SABS_966)',
  })
  ...
  @ApiOperation({
    summary: 'Get all EN 1452 PVC-U pipe specifications (static data)',
  })
  @ApiResponse({
    status: 200,
    description: 'Returns all EN 1452 specifications',
  })
  ...
  getEN1452Specification(
    ...
    if (!spec) {
      return {
        error: `Specification for DN ${dn} and PN ${pn} is not available`,
      };
    }
    return spec;
  }
  ...
  async getPipeSpecificationsByDN(
  ...
  @ApiQuery({
    name: 'pvcType',
    required: false,
    description: 'PVC type (default: PVC-U)',
  })
  async getPipeSpecification(
    ...
    return this.pvcService.getPipeSpecification(
      nominalDiameter,
      pressureRating,
      pvcType,
    );
  ...
  @ApiParam({
    name: 'code',
    description: 'Fitting type code (e.g., elbow_90, tee)',
  })
  ...
  async calculatePipeCostGet(
    ...
    const dto: CalculatePvcPipeCostDto = {
      ...
      cementJointPrice: cementJointPrice
        ? parseFloat(String(cementJointPrice))
        : undefined,
    };
    return this.pvcService.calculatePipeCost(dto);
  }
  ...
  async calculateFittingCostGet(
    ...
    const dto: CalculatePvcFittingCostDto = {
      ...
      pressureRating: pressureRating
        ? parseFloat(String(pressureRating))
        : undefined,
      cementJointPrice: cementJointPrice
        ? parseFloat(String(cementJointPrice))
        : undefined,
  ...
  async calculateTotalTransportWeight(
    @Body() dto: CalculatePvcTotalTransportDto,
  ) {
  ...
  @ApiResponse({
    status: 200,
    description: 'Returns list of available nominal diameters',
  })
  ...
  @ApiOperation({
    summary: 'Get available pressure ratings for a nominal diameter',
  })
  @ApiParam({ name: 'nominalDiameter', description: 'Nominal diameter in mm' })
