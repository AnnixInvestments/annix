...
export class WorkflowService {
  ...
  async submitForReview(
    ...
    await this.workflowRepository.deactivateAllForEntity(entityType, entityId);

    const saved = await this.workflowRepository.create({
  ...
  async approve(workflowId: number, notes: string, user: User): Promise<ReviewWorkflow> {
  ...
  async reject(workflowId: number, reason: string, user: User): Promise<ReviewWorkflow> {
    return this.processDecision(
      workflowId,
      ReviewStatus.REJECTED,
      reason,
  ...
  async findOne(id: number): Promise<ReviewWorkflow> {
    const workflow = await this.workflowRepository.findWithRelations(id);
    if (!workflow) {
  ...
  findActiveByEntity(
    ...
    return this.workflowRepository.findActiveByEntity(entityType, entityId);
  }

  getPendingReviews(reviewerUserId: number): Promise<ReviewWorkflow[]> {
    return this.workflowRepository.findPendingForReviewer(reviewerUserId);
  }

  findAll(query: WorkflowQueryDto): Promise<PaginatedResult<ReviewWorkflow>> {
    return this.workflowRepository.findAllPaginated(query);
  }
