...
- A clause is a real definition (and SHOULD be included here) when this document gives ANY concrete values: thicknesses, primer/topcoat names, DFT ranges, hardness, NDT %, pH ranges, surface prep grades, material standards, dimensions, ratings. EVEN IF other sibling docs also reference the same code, the definition belongs on the doc that actually defines it. DO NOT skip a definition just because the code is mentioned elsewhere.
- Filename / heading hints like "Index", "Reference", "Schedule", or "Class List" do NOT make a document a pointer-only doc. Inspect the actual content. A 'Material Class Index' that lists each class with its standard / rating IS a definition document for those classes — extract them all.
- ONLY return specifications={} when the document genuinely contains zero definitions and is purely narrative (a transmittal, a scope-of-work pointing entirely at other docs, a cover sheet). When in doubt, EXTRACT — empty output is worse than a slightly over-eager extraction.
