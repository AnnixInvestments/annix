...
export const formatRelative = (iso: string | Date | null | undefined): string | null => {
  if (!iso) return null;
  if (iso instanceof Date) {
...
export const formatShortDate = (iso: string | Date | null | undefined): string => {
  if (!iso) return "";
  if (iso instanceof Date) {
...
export const formatISODate = (iso: string | Date | null | undefined): string => {
  if (!iso) return "";
  if (iso instanceof Date) {
...
export const formatDateLongZA = (iso: string | Date | null | undefined): string => {
  if (!iso) return "";
  if (iso instanceof Date) {
    return DateTime.fromJSDate(iso).toFormat("EEEE, d MMMM yyyy");
  }
...
export const daysBetween = (start: string | Date, end: string | Date): number => {
  const startDt = start instanceof Date ? DateTime.fromJSDate(start) : DateTime.fromISO(start);
  const endDt = end instanceof Date ? DateTime.fromJSDate(end) : DateTime.fromISO(end);
