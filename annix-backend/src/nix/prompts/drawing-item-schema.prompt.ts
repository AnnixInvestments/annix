...
2. Use these EXACT field names (camelCase, no aliases, no variants):
   ...
   - itemType (one of: pipe | bend | reducer | tee | manifold | flange | expansion_joint | tank_chute | valve | pump | instrument | consumable | other). Use "valve" for ANY valve (diaphragm, gate, knife-gate, ball, butterfly, pinch, globe, plug, check / non-return / NRV, air-release, pressure-reducing, etc.) — put the specific pattern in the description, do NOT downgrade a valve to "other". Use "consumable" for erection / installation hardware sold by quantity or the set — bolts, nuts, washers, stud sets and gaskets (full spec in the description). Use "instrument" for in-line instrumentation — flow meters, densitometers, pressure / temperature transmitters, gauges, level switches / transmitters, samplers — and keep any "by Instrumentation / by others / by client" supply-responsibility note verbatim in the description. Use "pump" for pumps. Use "other" only when the item genuinely fits none of the categories above. SUPPLY SCOPE for valves & pumps: on a fabrication / piping / spool drawing these are normally BOUGHT-OUT items supplied by the client or a valve / pump vendor, not the fabricator — unless a drawing note or covering document EXPLICITLY places valve / pump SUPPLY in the fabricator's scope, append "(bought-out — confirm supply scope)" to the description so a quoter treats it as a reference line, not a priceable fabrication item (still type it "valve" / "pump").
   - quantity (number)
