...
export class DocumentAnnotationService {
  ...
  async convertPdfToImages(buffer: Buffer, scale: number = 2.0): Promise<PdfPagesResponseDto> {
    ...
    try {
      ...
      this.logger.warn("PDF may contain unsupported graphics. Returning empty pages.");
      return { totalPages: 0, pages: [] };
    } finally {
      try {
        const files = await fs.readdir(tempDir);
  ...
  async extractFromRegion(
    ...
    try {
      ...
      this.logger.log(
        `[extractFromRegion] Buffer type: ${typeof buffer}, isBuffer: ${Buffer.isBuffer(buffer)}, length: ${buffer?.length ?? "undefined"}`,
      );
      ...
      if (!outputExists) {
        this.logger.error(
          `[extractFromRegion] Ghostscript did not create output file at ${outputPath}`,
        );
        return { text: "", confidence: 0 };
      }
      ...
      this.logger.log(
        `[extractFromRegion] Page image buffer size: ${pageImageBuffer.length} bytes`,
      );

      const imageMetadata = await sharp(pageImageBuffer).metadata();
      this.logger.log(
        `[extractFromRegion] Image dimensions: ${imageMetadata.width}x${imageMetadata.height}`,
      );
      this.logger.log(
        `[extractFromRegion] Region to extract: x=${regionCoordinates.x}, y=${regionCoordinates.y}, w=${regionCoordinates.width}, h=${regionCoordinates.height}`,
      );
      ...
      if (
        regionCoordinates.x < 0 ||
        regionCoordinates.y < 0 ||
        regionRight > imgWidth ||
        regionBottom > imgHeight
      ) {
        this.logger.warn(
          `[extractFromRegion] Region extends beyond image bounds! Region: (${regionCoordinates.x},${regionCoordinates.y}) to (${regionRight},${regionBottom}), Image: ${imgWidth}x${imgHeight}`,
        );
      }
      ...
      this.logger.log(
        `[extractFromRegion] Extracted text: "${text.substring(0, 100)}${text.length > 100 ? "..." : ""}"`,
      );
