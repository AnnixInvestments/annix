...
Use null when a field is genuinely absent from the document. Never invent values.`;
  ...
  private liningPrompt(): string {
    return [
      ...
      "",
      "- `manufacturer`: the company that makes the product (e.g. 'Linatex', 'Weir', 'Trelleborg', 'Polycorp', 'AU Rubber'). Just the company name.",
      "- `productName`: the single product this sheet describes (e.g. 'Linard 60', 'Linatex Premium', 'Skega 60', 'Polylast 70'). Include the grade/series identifier so the library can distinguish 'Linard 60' from 'Linard 70'.",
      ...
      "- `description`: a single comma-separated line of THIS PRODUCT'S properties, drawn ONLY from what the sheet states. Include each field only if the sheet states it, in this order: hardness (e.g. '60 Shore A'), cure method (e.g. 'steam cured', 'autoclave vulcanised', 'press cured'), bonding compatibility (e.g. 'hot-bonded', 'cold-bonded'), polymer family (e.g. 'natural rubber', 'NBR', 'EPDM', 'butyl'), specific gravity if given, tensile strength if given, abrasion resistance class if given, colour (e.g. 'red', 'black', 'tan').",
      "",
      "Examples:",
      "  Linatex / Linard 60: { manufacturer: 'Linatex', productName: 'Linard 60', publishedRevision: 'Rev 03', publishedDate: '2024-08', brand: null, description: '60 Shore A, steam cured, hot-bonded, natural rubber, red' }",
      ...
      "",
      "Important: bore thickness and flange thickness are application-specific (decided by the quoter) — do NOT extract them from the data sheet.",
      "",
