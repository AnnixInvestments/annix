...
export class PipeEndConfigurationService {
  ...
  async getFlangeConfiguration(configCode: string): Promise<FlangeConfiguration> {
    const config = await this.findByCode(configCode);
  ...
  async getBoltSetCount(configCode: string, hasEqualBranch: boolean = true): Promise<BoltSetCount> {
    const config = await this.findByCode(configCode);
  ...
  hasLooseFlange(configCode: string): boolean {
    return configCode.includes("_LF") || configCode.includes("FOE_LF") || configCode === "2xLF";
  }
  ...
  formatCombinedEndConfig(mainEndConfig: string, stubs: StubFlangeConfig[]): string {
