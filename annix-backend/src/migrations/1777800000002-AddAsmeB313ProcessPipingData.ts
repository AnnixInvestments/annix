...
export class AddAsmeB313ProcessPipingData1777800000002 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    const miterBendData = [
      ...
      ["multiple_miter", 22.5, "normal", "equation_4a", "Per cut angle for multiple miters"],
      [
        "single_miter",
        22.5,
        "severe_cyclic",
    ...
    const bendFlatteningData = [
      ["general_service", 8.0, "Flattening shall not exceed 8% of outside diameter"],
      ["severe_cyclic", 5.0, "Flattening shall not exceed 5% for severe cyclic conditions"],
    ];
    ...
    const sifData = [
      ...
      ["slip_on_flange", "1.2", "1.2", "1.0", "Higher SIF due to socket detail"],
      ["socket_weld_flange", "1.3", "1.3", "1.0", "Higher SIF due to socket detail"],
    ...
    const ndeData = [
      ...
      ["severe_cyclic", "radiographic", 100, "Severe cyclic conditions - 100% RT or UT"],
      ["high_pressure", "radiographic", 100, "High pressure piping - 100% RT or UT"],
      ["category_m", "liquid_penetrant", 100, "Surface examination for toxic service"],
      ["severe_cyclic", "liquid_penetrant", 100, "Surface examination for cyclic service"],
    ];
    ...
    const flangeWeldData = [
      ...
      ["socket_weld", "fillet_weld", "outside", "leg = 1.09 × tn", "1.5mm expansion gap required"],
      ["lap_joint", "fillet_weld", "outside", "throat = min(tn, Tf)", "Weld to stub end only"],
    ];
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query("DROP TABLE IF EXISTS stress_intensification_factors");
