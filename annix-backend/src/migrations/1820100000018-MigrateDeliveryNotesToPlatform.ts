...
export class MigrateDeliveryNotesToPlatform1820100000018 implements MigrationInterface {
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(`
      CREATE INDEX IF NOT EXISTS idx_platform_dn_company ON platform_delivery_notes(company_id)
    `);
    await queryRunner.query(`
      CREATE INDEX IF NOT EXISTS idx_platform_dn_legacy_sc ON platform_delivery_notes(legacy_sc_delivery_note_id) WHERE legacy_sc_delivery_note_id IS NOT NULL
    `);
    await queryRunner.query(`
      CREATE INDEX IF NOT EXISTS idx_platform_dn_legacy_rubber ON platform_delivery_notes(legacy_rubber_delivery_note_id) WHERE legacy_rubber_delivery_note_id IS NOT NULL
    `);
    ...
    await queryRunner.query(`
      CREATE INDEX IF NOT EXISTS idx_platform_dni_dn ON platform_delivery_note_items(delivery_note_id)
    `);
  }
