...
export class AddAsmeVIIIPressureVesselData1777800000004
  implements MigrationInterface
{
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    const weldCategories = [
      ['A', 'Longitudinal', 'Longitudinal joints in shells, communicating chambers, nozzles, and any joint within a sphere'],
      ['B', 'Circumferential', 'Circumferential joints in shells and nozzles; joints between hemispherical heads and shells'],
      ['C', 'Flange-to-shell', 'Joints connecting flanges, tubesheets, flat heads to main shell, nozzles, or communicating chambers'],
      ['D', 'Nozzle-to-shell', 'Joints connecting nozzle necks, communicating chambers, or couplings to shells, heads, or flat-sided vessels'],
    ];
    ...
    const jointEfficiencies = [
      ...
      [1, 'Double-welded butt joint without radiography', 0.70, 0.70, 0.70, 0.70, 'No RT'],
      [2, 'Single-welded butt with backing strip, full RT', 0.90, 0.90, 0.90, 0.90, 'Full RT'],
      [2, 'Single-welded butt with backing strip, spot RT', 0.80, 0.80, 0.80, 0.80, 'Spot RT'],
      [2, 'Single-welded butt with backing strip, no RT', 0.65, 0.65, 0.65, 0.65, 'No RT'],
      [3, 'Single-welded butt without backing strip', null, 0.80, null, null, 'Spot RT'],
      [3, 'Single-welded butt without backing strip, no RT', null, 0.60, null, null, 'No RT'],
      [4, 'Double full fillet lap joint', null, 0.55, null, null, 'No RT'],
      [5, 'Single full fillet lap joint with plug welds', null, 0.50, null, null, 'No RT'],
      [6, 'Single-welded butt without backing (Cat B only)', null, 0.60, null, null, 'No RT'],
    ];
    ...
    const headTypes = [
      ['hemispherical', 'Hemispherical head - half sphere', 0.5, 't = PR/(2SE - 0.2P)', 'High pressure vessels, reactors'],
      ['ellipsoidal_2_1', 'Ellipsoidal head - 2:1 ratio', 1.0, 't = PD/(2SE - 0.2P)', 'Standard pressure vessels'],
      ['torispherical', 'Torispherical (ASME F&D) head', 1.77, 't = 0.885PL/(SE - 0.1P)', 'Low pressure vessels, tanks'],
      ['flat', 'Flat head or cover', null, 'Per UG-34 (C factor method)', 'Small diameter, low pressure'],
      ['conical', 'Conical head or reducer', null, 't = PD/(2cos(α)(SE - 0.6P))', 'Transition sections, hoppers'],
      ['toriconical', 'Toriconical head', null, 'Combination torispherical/conical', 'Large diameter transitions'],
    ];
    ...
    const gasketData = [
      ...
      ['rubber_without_fabric', 0.50, 0, 0, 'Plain elastomers, O-rings'],
      ['compressed_fiber', 2.00, 1600, 11.03, 'Compressed non-asbestos fiber'],
      ['spiral_wound_metal', 2.50, 10000, 68.95, 'Spiral wound with metal windings'],
      ['spiral_wound_ptfe', 2.00, 3700, 25.51, 'Spiral wound with PTFE filler'],
      ['flexible_graphite', 2.00, 3700, 25.51, 'Flexible graphite sheet'],
      ['soft_aluminum', 2.75, 4000, 27.58, 'Soft aluminum (O temper)'],
      ['soft_copper_brass', 3.00, 4500, 31.03, 'Soft copper or brass'],
      ['iron_soft_steel', 3.25, 5500, 37.92, 'Iron or soft steel'],
      ['monel_4_6_chrome', 3.50, 6500, 44.82, 'Monel, 4-6% chrome steel'],
      ['stainless_steel', 3.75, 7600, 52.40, 'Stainless steel'],
      ['ring_joint_soft_iron', 5.50, 18000, 124.11, 'Ring joint - soft iron/carbon steel'],
      ['ring_joint_monel', 6.00, 21800, 150.31, 'Ring joint - Monel/4-6% chrome'],
      ['ring_joint_stainless', 6.50, 26000, 179.27, 'Ring joint - stainless steel'],
    ];
    ...
    const pwhtData = [
      ['carbon_steel', 'P-1', 32, 1.25, 1100, 1200, 593, 649, '1 hr/in., 15 min minimum', 'Standard carbon steel vessels'],
      ['low_alloy', 'P-3', 16, 0.625, 1100, 1200, 593, 649, '1 hr/in., 15 min minimum', 'C-Mo, Mn-Mo steels'],
      ['cr_mo_low', 'P-4', 13, 0.5, 1250, 1300, 677, 704, '1 hr/in., 15 min minimum', '1-1/4Cr-1/2Mo, 2Cr-1/2Mo'],
      ['cr_mo_high', 'P-5A', 13, 0.5, 1300, 1400, 704, 760, '1 hr/in., 15 min minimum', '5Cr-1/2Mo through 9Cr-1Mo'],
      ['stainless_austenitic', 'P-8', null, null, null, null, null, null, 'Not required unless specified', '304, 316 stainless - solution anneal if required'],
    ];

    for (const [material, pNum, thickMm, thickIn, tempMinF, tempMaxF, tempMinC, tempMaxC, holding, notes] of pwhtData) {
      await queryRunner.query(
        `
    ...
    const testReqs = [
      ['hydrostatic', 1.30, 'PT = 1.3 × MAWP × (ST/S)', 'Time for examination', 'Standard test method, preferred'],
      ['pneumatic', 1.10, 'PT = 1.1 × MAWP × (ST/S)', '10 minutes minimum', 'Increase in stages: 25%, then 10% increments'],
      ['hydropneumatic', 1.30, 'Combination hydro + pneumatic', 'Per procedure', 'Used when complete filling not practical'],
    ];
    ...
    const materialSpecs = [
      ...
      ['SA-312', 'pipe', 'TP304, TP316, TP321, TP347', 'Seamless and welded stainless steel pipe', null],
      ['SA-333', 'pipe', '1, 6, 7, 8', 'Seamless and welded pipe for low temperature', null],
      ['SA-335', 'pipe', 'P5, P9, P11, P22', 'Seamless alloy steel pipe for high temperature', null],
      ['SA-105', 'forging', 'Standard', 'Carbon steel forgings for flanges and fittings', null],
      ['SA-182', 'forging', 'F304, F316, F11, F22', 'Forged or rolled alloy and stainless flanges/fittings', null],
      ['SA-350', 'forging', 'LF1, LF2, LF3', 'Carbon and low alloy forgings for low temperature', null],
      ['SA-266', 'forging', '1, 2, 3, 4', 'Carbon steel forgings for pressure vessel components', null],
    ];
    ...
    const coneAngles = [
      ...
      [60, 'Knuckle or reinforcement required at large end', true, '6% of large diameter or reinforcement per Appendix 1-5'],
    ];
    ...
    const flangeTypes = [
      ['integral', 'One-piece forged or welded construction with hub', 'Appendix 2', '2-3, 2-4', 'Highest integrity, used for high pressure'],
      ['loose', 'Lapped, threaded, or slip-on construction', 'Appendix 2', '2-5', 'Lower cost, easier alignment'],
      ['optional', 'Can be designed as integral when meeting requirements', 'Owner choice', '2-6', 'Designer option for borderline cases'],
      ['reverse', 'Bolts inside, gasket outside bolt circle', 'Special rules', '2-10', 'Used for heat exchangers, special applications'],
      ['full_face', 'Gasket extends to outer edge', 'Appendix 2', '2-5', 'Typically with flat face flanges'],
      ['split_loose', 'Multi-piece loose flange', 'Appendix 2', '2-5', 'Large diameter applications'],
    ];
