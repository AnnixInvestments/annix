...
export class AddM38BoltAndBs4504SpecialFlanges1776200000000 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    console.warn(
      'Adding M38 bolt and BS4504 special flanges for large NB sizes...',
    );

    const existingM38 = await queryRunner.query(
      `SELECT id FROM bolts WHERE designation = 'M38'`,
    );
    ...
    const insertFlange = async (
      ...
      if (existing.length > 0) {
        console.warn(
          `${nb}NB BS4504 PN${pressureClass}${typeCode} already exists, skipping...`,
        );
        return;
      }
    ...
    const ensurePressureClass = async (
      pressureClass: number,
      typeCode: string,
    ) => {
