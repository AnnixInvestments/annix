...
export class AddFlangeDataRelationships1775400000000 implements MigrationInterface {
  ...
  private async createIntegralFlangeCompatibility(
    ...
    const asmeB1647A = await queryRunner.query(
      `SELECT id FROM flange_standards WHERE code = 'ASME B16.47 Series A'`,
    );
    const asmeB1647B = await queryRunner.query(
      `SELECT id FROM flange_standards WHERE code = 'ASME B16.47 Series B'`,
    );
    const sabs1123 = await queryRunner.query(
      `SELECT id FROM flange_standards WHERE code = 'SABS 1123'`,
    );
    const bs4504 = await queryRunner.query(
      `SELECT id FROM flange_standards WHERE code = 'BS 4504'`,
    );
    const bs10 = await queryRunner.query(
    ...
    const class900 = await queryRunner.query(
      `SELECT id FROM flange_pressure_classes WHERE designation = 'Class 900' AND "standardId" = $1`,
      [asmeB165Id],
    );
    const class1500 = await queryRunner.query(
      `SELECT id FROM flange_pressure_classes WHERE designation = 'Class 1500' AND "standardId" = $1`,
      [asmeB165Id],
    );
    ...
    const compatibilityData = [
      {
        nps: '1/2',
        schedule: 'STD',
        standardId: asmeB165Id,
        typeId: wnTypeId,
        classId: class150Id,
        minWall: 2.77,
        maxWall: null,
        notes: 'Standard weight - suitable for Class 150 WN',
      },
      {
        nps: '1/2',
        schedule: 'XS',
        ...
        minWall: 3.73,
        maxWall: null,
        notes: 'Extra strong - suitable for Class 300 WN',
      },
      {
        nps: '3/4',
        schedule: 'STD',
      ...
      },
      {
        ...
        minWall: 6.35,
        maxWall: null,
        notes: 'Schedule 160 for Class 600',
      },
      {
        nps: '1-1/2',
        schedule: 'STD',
...

      {
        ...
        minWall: 3.73,
        maxWall: null,
        notes: 'Socket weld for small bore high pressure',
      },
      {
        nps: '3/4',
        schedule: 'Sch 80',
...

      {
        ...
        minWall: 2.77,
        maxWall: null,
        notes: 'Slip-on suitable for low pressure',
      },
      {
        nps: '3/4',
        schedule: 'STD',
...

      {
        nps: '26',
        schedule: 'STD',
        standardId: asmeB1647AId,
        typeId: wnTypeId,
        classId: class150Id,
        minWall: 9.53,
        maxWall: null,
        notes: 'B16.47 Series A for large bore',
      },
      {
        nps: '28',
        schedule: 'STD',
...

      {
        nps: '26',
        schedule: 'STD',
        standardId: asmeB1647BId,
        typeId: wnTypeId,
        classId: class150Id,
        minWall: 9.53,
        maxWall: null,
        notes: 'B16.47 Series B (API 605) for large bore',
      },
      {
        nps: '28',
        schedule: 'STD',
    ...
    console.warn(
      `Added ${compatibilityData.length} integral flange compatibility records`,
    );
  }
  ...
  private async addFaceFinishField(queryRunner: QueryRunner): Promise<void> {
    ...
    const faceFinishes = [
      {
        code: 'RF',
        name: 'Raised Face',
        description: 'Standard raised face finish for general service',
        raMin: 3.2,
        raMax: 6.3,
        gaskets: ['spiral wound', 'PTFE', 'graphite', 'rubber'],
        standard: 'ASME B16.5',
      },
      {
        code: 'FF',
        name: 'Flat Face',
        description: 'Full face for cast iron and non-metallic flanges',
        raMin: 3.2,
        raMax: 6.3,
        gaskets: ['full face rubber', 'PTFE', 'fiber'],
        standard: 'ASME B16.5',
      },
      {
        code: 'RTJ',
        name: 'Ring Type Joint',
        description: 'Metallic ring groove for high pressure/temperature',
        raMin: 0.8,
        raMax: 1.6,
        gaskets: ['ring joint R', 'ring joint RX', 'ring joint BX'],
        standard: 'ASME B16.5/API 6A',
      },
      {
        code: 'SERR',
        name: 'Serrated',
        description: 'Concentric or spiral serrations for enhanced sealing',
        raMin: 3.2,
        raMax: 12.5,
        gaskets: ['spiral wound', 'graphite', 'PTFE'],
        standard: 'ASME B16.5',
      },
      {
        code: 'SMOOTH',
        name: 'Smooth',
        description: 'Stock/as-machined finish 125-250 AARH',
        raMin: 3.2,
        raMax: 6.3,
        gaskets: ['spiral wound', 'PTFE', 'graphite'],
        standard: 'ASME B16.5',
      },
      {
        code: 'GROOVED',
        name: 'Grooved',
        description: 'Phonographic spiral groove',
        raMin: null,
        raMax: null,
        gaskets: ['solid metal', 'spiral wound'],
        standard: 'ASME B16.5',
      },
      {
        code: 'LAPPED',
        name: 'Lapped',
        description: 'Smooth lapped finish for lap joint flanges',
        raMin: 1.6,
        raMax: 3.2,
      ...
      },
      {
        code: 'MALE',
        name: 'Male',
        description: 'Tongue side of tongue-and-groove joint',
        raMin: 3.2,
        raMax: 6.3,
        gaskets: ['solid metal', 'spiral wound'],
        standard: 'ASME B16.5',
      },
      {
        code: 'FEMALE',
        name: 'Female',
        description: 'Groove side of tongue-and-groove joint',
        raMin: 3.2,
        raMax: 6.3,
        gaskets: ['solid metal', 'spiral wound'],
        standard: 'ASME B16.5',
    ...
    if (columnExists.length === 0) {
      ...
      const rfFinish = await queryRunner.query(
        `SELECT id FROM flange_face_finishes WHERE code = 'RF'`,
      );
  ...
  private async createTestFlangesTable(
    queryRunner: QueryRunner,
  ): Promise<void> {
    ...
    const rtjFinish = await queryRunner.query(
      `SELECT id FROM flange_face_finishes WHERE code = 'RTJ'`,
    );
    ...
    const testFlanges = [
      {
        ...
        finishId: rfFinishId,
        maxPressure: 20.7,
        notes: 'Standard mock-up for hydro test',
      },
      {
        name: 'Mock-Up Ring 4" CL150',
        type: 'MOCK_UP_RING',
        nps: '4',
        class: 'Class 150',
        standardId: asmeB165Id,
...

      {
        ...
        finishId: rfFinishId,
        maxPressure: 51.7,
        notes: 'Class 300 mock-up for hydro test',
      },
      {
        name: 'Mock-Up Ring 4" CL300',
        type: 'MOCK_UP_RING',
        nps: '4',
        class: 'Class 300',
        standardId: asmeB165Id,
...

      {
        ...
        finishId: rfFinishId,
        maxPressure: 20.7,
        notes: 'Solid blind for test isolation',
      },
      {
        name: 'Test Blind 6" CL150',
        type: 'TEST_BLIND',
        nps: '6',
        class: 'Class 150',
        standardId: asmeB165Id,
      ...
      {
        name: 'Spectacle Blind 4" CL150',
        type: 'SPECTACLE_BLIND',
        nps: '4',
        class: 'Class 150',
        standardId: asmeB165Id,
        material: 'Carbon Steel',
        ...
        finishId: rfFinishId,
        maxPressure: 20.7,
        notes: 'Figure-8 spectacle blind per ASME B16.48',
      },
      {
        name: 'Spectacle Blind 6" CL150',
        type: 'SPECTACLE_BLIND',
        nps: '6',
        class: 'Class 150',
        standardId: asmeB165Id,
...

      {
        ...
        numBolts: 8,
        boltDia: 25.4,
        finishId: rtjFinishId,
        maxPressure: 103.4,
        notes: 'RTJ face mock-up for high pressure',
      },
      {
        name: 'RTJ Mock-Up 6" CL600',
        type: 'MOCK_UP_RING',
        nps: '6',
        class: 'Class 600',
        standardId: asmeB165Id,
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `DROP TABLE IF EXISTS test_flange_usage_log CASCADE`,
    );
    ...
    if (columnExists.length > 0) {
      await queryRunner.query(
        `ALTER TABLE flange_dimensions DROP COLUMN face_finish_id`,
      );
    }

    await queryRunner.query(
      `DROP TABLE IF EXISTS flange_face_finishes CASCADE`,
    );
    await queryRunner.query(
      `DROP TABLE IF EXISTS integral_flange_compatibility CASCADE`,
    );
