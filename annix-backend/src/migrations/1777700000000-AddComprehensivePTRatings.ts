...
export class AddComprehensivePTRatings1777700000000 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    console.warn('Adding comprehensive P-T ratings for all flange standards...');
    ...
    if (bs4504Result.length > 0) {
      ...
      for (const cls of bs4504Classes) {
        ...
        for (const [temp, factor] of carbonSteelDerating) {
          await insertPtRating(cls.id, 'Carbon Steel', temp, Math.round(basePN * factor * 10) / 10);
        }

        // Carbon Steel A105 (Group 1.1) - same as Carbon Steel
        for (const [temp, factor] of carbonSteelDerating) {
          await insertPtRating(cls.id, 'Carbon Steel A105 (Group 1.1)', temp, Math.round(basePN * factor * 10) / 10);
        }

        // Stainless Steel 304
        for (const [temp, factor] of ss304Derating) {
          await insertPtRating(cls.id, 'Stainless Steel 304 (Group 2.1)', temp, Math.round(basePN * factor * 10) / 10);
        }

        // Stainless Steel 316
        for (const [temp, factor] of ss316Derating) {
          await insertPtRating(cls.id, 'Stainless Steel 316 (Group 2.2)', temp, Math.round(basePN * factor * 10) / 10);
        }
      }

      console.warn(`Added P-T ratings for ${bs4504Classes.length} BS 4504 pressure classes`);
    }
    ...
    if (sabs1123Result.length > 0) {
      ...
      const classesByBase: Record<string, { id: number; designation: string }[]> = {};
        ...
        if (sourceClassId) {
          ...
          for (const cls of classes) {
            ...
            for (const pt of ptData) {
              await insertPtRating(cls.id, pt.material_group, pt.temperature_celsius, pt.max_pressure_bar);
            }
          }
          console.warn(`Copied P-T data from SABS ${base}/3 to ${classes.length - 1} variants`);
        } else {
          // No source data - need to add base P-T data
          const basePressure = parseInt(base, 10) / 100; // 600 kPa = 6 bar, etc.
          const carbonSteelDerating: [number, number][] = [
            [-10, 1.00], [20, 1.00], [50, 1.00], [100, 1.00],
            [150, 0.93], [200, 0.85],
          ];

          for (const cls of classes) {
            for (const [temp, factor] of carbonSteelDerating) {
              await insertPtRating(cls.id, 'Carbon Steel', temp, Math.round(basePressure * factor * 10) / 10);
            }
          }
          console.warn(`Added base P-T data for SABS ${base} (${classes.length} classes)`);
        }
      }
    }
    ...
    if (asmeB165Result.length > 0) {
      ...
      for (const [base, classes] of Object.entries(classesByBase)) {
        ...
        if (sourceClassId) {
          ...
          if (copiedCount > 0) {
            console.warn(`Copied P-T data from ASME ${sourceDesignation} to ${copiedCount} variants`);
          }
        }
      }
    }
    ...
    for (const stdCode of ['ASME B16.47A', 'ASME B16.47B']) {
      const stdResult = await queryRunner.query(
        `SELECT id FROM flange_standards WHERE code = $1`, [stdCode]
      );
      ...
      if (stdResult.length > 0) {
        ...
        if (missingClasses.length > 0) {
          console.warn(`Adding P-T data for ${missingClasses.length} ${stdCode} classes...`);

          // Use ASME B16.5 derating factors
          const asmeDerating: Record<number, [number, number][]> = {
            150: [[-29, 19.6], [38, 19.6], [93, 18.0], [149, 15.8], [204, 13.8], [260, 12.1], [316, 10.2], [343, 8.8], [371, 7.4], [399, 6.1], [427, 4.9], [454, 3.9]],
            300: [[-29, 51.1], [38, 51.1], [93, 47.6], [149, 45.1], [204, 42.1], [260, 37.2], [316, 32.7], [343, 28.1], [371, 23.8], [399, 19.6], [427, 15.7], [454, 12.4]],
            600: [[-29, 102.1], [38, 102.1], [93, 95.1], [149, 90.2], [204, 84.1], [260, 74.5], [316, 65.5], [343, 56.2], [371, 47.6], [399, 39.3], [427, 31.4], [454, 24.8]],
            900: [[-29, 153.2], [38, 153.2], [93, 142.7], [149, 135.4], [204, 126.2], [260, 111.7], [316, 98.2], [343, 84.3], [371, 71.4], [399, 58.9], [427, 47.1], [454, 37.2]],
          };
          ...
          for (const cls of missingClasses) {
            ...
            if (data) {
              for (const [temp, pressure] of data) {
                await insertPtRating(cls.id, 'Carbon Steel A105 (Group 1.1)', temp, pressure);
              }
            }
          }
        }
