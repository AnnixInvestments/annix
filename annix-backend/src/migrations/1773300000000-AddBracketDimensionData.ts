...
export class AddBracketDimensionData1773300000000 implements MigrationInterface {
    ...
    for (const bracket of newBracketTypes) {
      ...
      `,
        [
          ...
          bracket.minNb,
          bracket.maxNb,
          bracket.weightFactor,
          bracket.baseCost,
          bracket.insulatedSuitable,
          bracket.allowsExpansion,
          bracket.isAnchorType,
        ],
      );
    }
    console.warn(
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `DELETE FROM bracket_types WHERE type_code IN ('SPRING_HANGER', 'CONSTANT_SUPPORT', 'RISER_CLAMP')`,
    );
  }
}
