...
export class AddASMEFlangeStandardsAndPTRatings1766001600000 implements MigrationInterface {
  public async up(queryRunner: QueryRunner): Promise<void> {
    console.warn("🔧 Adding ASME flange standards and comprehensive P-T ratings...");
    ...
    for (const code of asmeStandards) {
      ...
      if (existing.length === 0) {
        await queryRunner.query("INSERT INTO flange_standards (code) VALUES ($1)", [code]);
    ...
    console.warn("  ✓ Added ASME B16.5 pressure classes: 150, 300, 400, 600, 900, 1500, 2500");
    ...
    console.warn("  ✓ Added ASME B16.47 pressure classes: 75, 150, 300, 400, 600, 900");
    ...
    for (const { id } of classIds) {
      await queryRunner.query("DELETE FROM flange_pt_ratings WHERE pressure_class_id = $1", [id]);
    }
