...
export class CreateCoatingSpecificationTables1766001800000
  implements MigrationInterface
{
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    // Get standard IDs
    const iso12944 = await queryRunner.query(
      `SELECT id FROM "coating_standards" WHERE code = 'ISO 12944'`,
    );
    const norsok = await queryRunner.query(
      `SELECT id FROM "coating_standards" WHERE code = 'NORSOK M-501'`,
    );
