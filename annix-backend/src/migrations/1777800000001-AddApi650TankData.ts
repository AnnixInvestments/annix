...
export class AddApi650TankData1777800000001 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    const nozzleTypes = [
      ['SHELL', 'Shell Nozzle', 'Nozzle penetrating tank shell', '{SO,WN,LWN,RING,LAP}', 'Product inlet/outlet, drain, instrumentation'],
      ['ROOF', 'Roof Nozzle', 'Nozzle penetrating tank roof', '{SO,WN,LAP}', 'Venting, gauging, fill connections'],
      ['BOTTOM', 'Bottom Nozzle', 'Nozzle penetrating tank bottom', '{SO,WN,LAP}', 'Drain, outlet connections'],
      ['MANHOLE_SHELL', 'Shell Manhole', 'Access opening in tank shell', '{FLANGED}', 'Personnel and equipment access'],
      ['MANHOLE_ROOF', 'Roof Manhole', 'Access opening in tank roof', '{FLANGED}', 'Personnel access, inspection'],
    ];
    ...
    const flangeTypes = [
      ...
      ['LONG_WELD_NECK', 'Long Welding-neck Flange', 'LWN', 'tn or ts (lesser)', 'Extended nozzle design', 19.0],
      ['RING_TYPE', 'Ring-type Flange', 'RING', 'tn or Tf (lesser)', 'Fillet weld both sides', null],
      ['LAP_JOINT', 'Lap Joint Flange', 'LAP', 'tn or Tf (lesser)', 'Easy alignment applications', null],
    ...
    for (const [type, sizeMm, sizeIn, coverMm, flangeMm, neckMm, boltCircle, numBolts, boltSize] of manholeSpecs) {
      await queryRunner.query(`
    ...
    for (const [nps, npsDecimal, schedule, wallMm, padMm, pipeOd, flangeOd, boltCircle, numBolts, boltSize] of nozzleDimensions) {
      await queryRunner.query(`
      ...
      `, [nps, npsDecimal, schedule, wallMm, padMm, pipeOd, flangeOd, boltCircle, numBolts, boltSize]);
    }
    ...
    const materialGroups = [
      ...
      ['IIA', 'Group IIA', 'Carbon steel, as-rolled (enhanced)', -29, 340, true, false, '{ASTM A516}'],
      ['III', 'Group III', 'Carbon-manganese steel', -46, 340, true, false, '{ASTM A537}'],
      ['IIIA', 'Group IIIA', 'Carbon-manganese steel, QT', -46, 400, true, false, '{ASTM A537 Cl 2}'],
    ...
    for (const [code, name, desc, minTemp, maxTemp, impactReq, seamlessReq, specs] of materialGroups) {
      await queryRunner.query(`
    ...
    const pipeSpecs = [
      ...
      ['A106', 'B', 'Seamless Carbon Steel Pipe for High-Temperature', 'High temperature service', -29, 400, 'II'],
      ['A333', '6', 'Seamless and Welded Steel Pipe for Low-Temperature', 'Low temperature service', -46, 340, 'III'],
      ['A524', 'I', 'Seamless Carbon Steel Pipe for Atmospheric', 'Atmospheric service', -29, 340, 'II'],
      ['A524', 'II', 'Seamless Carbon Steel Pipe for Atmospheric', 'Atmospheric service', -29, 340, 'II'],
      ['A671', 'CC60', 'Electric-Fusion-Welded Steel Pipe', 'Large diameter service', -29, 340, 'II'],
    ...
    const roofColumns = [
      ...
      ['STRUCTURAL', 'Wide flange or channel', null, false, false, 'Back-to-back channels or wide flange'],
    ];
    ...
    const weldReqs = [
      ['nozzle_to_shell_outside', 'fillet', '0.7 × tn minimum', false, null, 'Primary structural weld'],
      ...
      ['reinforcing_pad_to_shell', 'fillet', 'pad thickness', false, null, 'Continuous fillet weld'],
    ];
