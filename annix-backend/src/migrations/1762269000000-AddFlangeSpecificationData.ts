...
export class AddFlangeSpecificationData1762269000000 implements MigrationInterface {
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    for (const bolt of boltInserts) {
      ...
      if (existing.length === 0) {
        await queryRunner.query(
          `INSERT INTO bolts (designation) VALUES ('${bolt}')`,
        );
      }
    }
    ...
    for (const standard of standards) {
      ...
      if (existing.length === 0) {
        await queryRunner.query(
          `INSERT INTO flange_standards (code) VALUES ('${standard}')`,
        );
      }
    }
    ...
    const getPressureClassId = async (
      designation: string,
      standardId: number,
    ) => {
      const result = await queryRunner.query(`
    ...
    for (const flange of flangeData) {
      ...
      const nominalId = await getNominalId(
        nominalMm as number,
        outsideMm as number,
      );
      if (!nominalId) {
