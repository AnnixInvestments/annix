...
export class MigrateInvoicesToPlatform1820100000019 implements MigrationInterface {
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(`
      CREATE INDEX IF NOT EXISTS idx_platform_inv_company ON platform_invoices(company_id)
    `);
    await queryRunner.query(`
      CREATE INDEX IF NOT EXISTS idx_platform_inv_legacy_sc ON platform_invoices(legacy_sc_invoice_id) WHERE legacy_sc_invoice_id IS NOT NULL
    `);
    await queryRunner.query(`
      CREATE INDEX IF NOT EXISTS idx_platform_inv_legacy_rubber ON platform_invoices(legacy_rubber_invoice_id) WHERE legacy_rubber_invoice_id IS NOT NULL
    `);
  }
