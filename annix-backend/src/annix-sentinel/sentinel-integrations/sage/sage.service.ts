...
export class SageService {
  ...
  async syncCompanyInfo(companyId: number): Promise<SageCompanyInfo> {
    ...
    const companyKey = `annix-sentinel-${companyId}`;
  ...
  private async refreshAccessToken(connection: AnnixSentinelSageConnection): Promise<string> {
    ...
    await sageRateLimiter.waitForSlot(`annix-sentinel-oauth:${connection.companyId}`);
  ...
  private async exchangeCodeForTokens(code: string): Promise<SageTokenResponse> {
    await sageRateLimiter.waitForSlot("annix-sentinel-oauth:exchange");
  ...
  private async sageApiRequest<T>(
    ...
    await sageRateLimiter.waitForSlot(companyKey ?? "annix-sentinel-default");
