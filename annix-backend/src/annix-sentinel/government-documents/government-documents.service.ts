...
@Injectable()
export class AnnixSentinelGovernmentDocumentsService {
  ...
  async syncDocument(documentId: number): Promise<AnnixSentinelGovernmentDocument> {
    ...
    const response = await fetch(doc.sourceUrl, {
      headers: { "User-Agent": "Annix-AnnixSentinel/1.0" },
      redirect: "follow",
    });
    ...
    const subPath = `${StorageArea.ANNIX_SENTINEL}/government-documents/${doc.category}`;
